...
import { ColaWordmarkInline } from '@/components/agent/cola-authored';
...
interface Props {
  ...
  onShowFullDay: () => void;
  /**
   * True only when the seller has done absolutely nothing yet — no contacts,
   ...
   * focuses it. Wired through cola-workspace.
   */
 ...
 *
 * Cascade: drafts (most urgent) → questions (next-most). Seller sees ONE
 ...
 * rotates out of view now. Sellers should never feel forced to act on
...
export function FocusCard({
  ...
  // seller rotates to a new card. Used to compute time-to-decision (an
  // important feedback signal — fast send = trust the model, slow send = the
  // model is making the seller think too hard). Per-card so a held card
  ...
  // Also reset the view timestamp — the seller is "first seeing" this item
  // now, even if it was queued an hour ago server-side. Decision latency is
  // the time the *seller* spent looking at the card, not the time the draft
  // sat in the queue.
  useEffect(() => {
  ...
  function hold() {
    ...
    // the seller from rotating. Questions don't get this ping (different
  ...
  // Items exist but seller has held them all in this session
  if (!current) {
    return (
      <FocusEmpty
...
function FocusWelcome({
  ...
  function handleTellMe() {
    onTellMeAboutLead?.('Hi Cola, my most recent lead is ');
  }
  ...
  return (
    <div className={cn(FOCUS_CARD_MAX, 'mx-auto text-center py-10')}>
      ...
      <p className={cn(BODY_MUTED, 'mt-2 max-w-md mx-auto leading-relaxed')}>
        I&apos;m <ColaWordmarkInline />. I track your deals, draft your follow-ups, and flag what&apos;s stuck &mdash; automatically. Start with a lead and I&apos;ll show you.
      </p>
