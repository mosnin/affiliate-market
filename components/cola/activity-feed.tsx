...
const ACTION_META: Record<string, { verb: string; icon: LucideIcon }> = {
  ...
  packet_drafted: { verb: 'drafted a product packet for', icon: Paperclip },
  ...
  // Lifecycle actions added with the manager rollup work
  demo_booked: { verb: 'booked a demo for', icon: CalendarCheck },
  deal_stage_advanced: { verb: 'moved a deal forward:', icon: ArrowUpRight },
  review_requested: { verb: 'flagged a deal for your manager:', icon: Flag },
  ...
  // Task lifecycle (logged when a seller pauses/resumes/cancels/completes
  // a Cola task). Phrased so the "I <verb> <target>" template still reads.
  ...
  return (
    <div className="space-y-6">
      {/* No tab strip — the product has no tabs anywhere else. Chronological
          by default is what sellers actually want; specific entries can be
      ...
      {!loading && entries.length === 0 && (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            When Cola acts, it&apos;ll show up here.
          </p>
        </div>
      )}
