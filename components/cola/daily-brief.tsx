 ...
 * "Today." greeting from ColaPageShell orients without competing.
 *
 * Sections:
 *   ON DECK   the seller's action cards (reuses brief.cards)
 ...
 *   OVERNIGHT what Cola did in the last 12h (from the same endpoint)
 */
...
interface Props {
   ...
   *  by the dedicated /cola/brief page where the seller navigated
...
export function DailyBrief({ slug, initialBrief, alwaysLive = false }: Props) {
  ...
  function recordActed(cardIndex?: number, source?: SignalSource, kind?: SignalKind) {
    ...
    // telemetry) or from the empty-state "Tell Cola" button (which
  ...
  // handling: the seller never stares at a blank surface wondering
  ...
  // so the "Yesterday →" toggle works whether the seller is in the live
...
/**
 * Sectioned command-center layout. ColaPageShell already provides the
...
/**
 * Day-one seller: no cards, no calendar events, no pipeline, no overnight
 * activity. Single quiet sentence + the same "Tell Cola" CTA the empty-
