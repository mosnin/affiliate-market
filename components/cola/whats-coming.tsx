 ...
 * Shows demos scheduled forward and follow-ups due today or overdue.
 * Hides itself when both lists are empty so quiet days stay calm.
 */
export function WhatsComing({ slug }: { slug: string }) {
  const [data, setData] = useState<TodayData>({ followUpsDue: [], demosUpcoming: [] });
      ...
      try {
        ...
        if (res.ok) {
          ...
          setData({
            ...
            demosUpcoming: json.demosUpcoming ?? [],
          });
        }
      } catch {
        // non-critical
  ...
  const total = data.followUpsDue.length + data.demosUpcoming.length;
    ...
    <section>
      ...
      {!loading && total > 0 && (
        <motion.div
          ...
          animate="enter"
        >
          {data.demosUpcoming.map((demo) => (
            <motion.div key={demo.id} variants={STAGGER_ITEM}>
              <Link
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-medium text-foreground tabular-nums">
                      {formatDay(demo.startsAt)} · {formatTime(demo.startsAt)}
                    </span>
                    {demo.guestName && (
                      <span className="text-muted-foreground truncate">with {demo.guestName}</span>
                    )}
                  </div>
                  {demo.productAddress && (
                    <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                      <MapPin size={10} />
                      <span className="truncate">{demo.productAddress}</span>
                    </p>
                  )}
                </div>
                <ChevronRight
