...
/**
 * ApprovalsPill — small status chip in the Cola header that shows the
 ...
 * page. The `/s/[slug]/cola/approvals` route still works as a deep link,
 * but the seller never has to navigate there.
 */
...
export function ApprovalsPill() {
  ...
  const fetchApprovals = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/cola/approvals');
      if (!res.ok) return;
  ...
  const handleAction = useCallback(
    async (taskId: string, action: 'approve' | 'reject') => {
      ...
      try {
        ...
        toast.success(action === 'approve' ? 'Approved. Cola will continue.' : 'Rejected. Action cancelled.');
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md p-0 flex flex-col">
          <SheetHeader className="px-5 py-4 border-b border-border/60">
            ...
            <SheetDescription>
              {count === 0
                ? 'Nothing waiting. Cola will ask before any risky action.'
                : count === 1
