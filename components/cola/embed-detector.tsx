 ...
 * when the page is loaded inside the Cola RightPanel iframe.
 *
 * Reads `?embed=1` from the URL and sets `data-cola-embed="true"` on
 ...
 *   - the persistent ColaBar (`[data-page-chat-input]`)
 *   - the mobile nav and platform banner (`[data-dashboard-mobile-nav]`,
 *     `[data-platform-banner]`)
 *
 * The outer Cola page already owns the sidebar, header, and chat input
...
export function EmbedDetector() {
  ...
  useEffect(() => {
    ...
    if (isEmbed) {
      root.setAttribute('data-cola-embed', 'true');
    } else {
      root.removeAttribute('data-cola-embed');
    }
    return () => {
      root.removeAttribute('data-cola-embed');
    };
