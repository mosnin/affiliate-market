...
import { ColaPromptBox, type MentionItem, type SkillItem } from '@/components/ui/cola-prompt-box';
...
import { PlanCard } from '@/components/cola/plan-card';
import { useSplitPanel } from '@/hooks/use-split-panel';
import { SplitPanelToggle } from '@/components/cola/split-panel-toggle';
import { RightPanel } from '@/components/cola/right-panel';
import { PanelResizeHandle } from '@/components/cola/panel-resize-handle';
import { ApprovalsPill } from '@/components/cola/approvals-pill';
...
interface ColaWorkspaceProps {
  ...
  initialInput?: string;
  /** Pre-populate the composer on mount but do NOT auto-send — the seller
   *  finishes the sentence themselves. Used by "or just tell Cola →"
   ...
   *  line under the post-demo affordance. Snapshot at page load: the seller
  ...
  skills?: SkillItem[];
  /**
   * Which Cola variant this surface is rendering.
   *
   * - `seller` (default) — solo / company-member chat at /s/<slug>/cola.
   *   Backed by `/api/ai/task` and the native seller tool catalog.
   * - `manager` — chief-of-staff chat at /manager/cola. Backed by the
   *   manager-gated `/api/ai/manager-task` (defense layer 2) and the
   *   `MANAGER_TOOLS` registry (empty in Phase 1; Phase 2/3 will populate).
   *
 ...
 *   3. localStorage flag `cola:sound:enabled === '1'`. Default OFF —
 *      sound is a delight the seller chooses to turn on, never one we
...
function softTap(): void {
  ...
  try {
    ...
    if (window.localStorage?.getItem('cola:sound:enabled') !== '1') return;
...
export function ColaWorkspace({
  ...
  // The single sentence the seller sees when they approve a celebrate-able
  ...
  const {
    ...
    // Manager variant routes through `/api/ai/manager-task` — the manager
    // version re-checks `resolveManagerContext()` (defense layer 2) before
    // forwarding the turn to Modal with `mode: 'manager'`.
    ...(isManager
      ? {
          taskEndpoint: '/api/ai/manager-task',
          conversationsEndpoint: '/api/ai/manager-conversations',
          // The manager conversations route reads company scope from the
    ...
    onConversationCreated: (id) => {
      ...
      // — seller sits at /s/<slug>/cola, manager at /manager/cola.
      const newConvBase = isManager ? '/manager' : `/s/${slug}/cola`;
  ...
  // Wrap approve / alwaysAllow so the moment the seller approves a
  ...
  useEffect(() => {
    if (searchParams.get('view') === 'history') {
      ...
      const baseUrl = isManager ? '/manager' : `/s/${slug}/cola`;
      router.replace(`${baseUrl}${qs ? `?${qs}` : ''}`, { scroll: false });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Manager history comes from the manager-gated `/api/ai/manager-messages`
  // (reads "ManagerMessage"); seller history from `/api/ai/messages` (reads
  // "Message"). Choosing by variant, mirroring taskEndpoint / conversations-
  // Endpoint above, is what fixes the manager "can't load history after reload"
  // 404 — the seller endpoint refuses manager conversations by design.
  const messagesEndpoint = isManager ? '/api/ai/manager-messages' : '/api/ai/messages';

  // Per-conversation mutations (rename / delete) hit the manager-gated
  // `/api/ai/manager-conversations/<id>` for the manager variant — the seller
  // `/api/ai/conversations/<id>` route refuses manager rows (and the manager
  // rows live in a different table entirely). Returns the base path; callers
  // append the conversation id.
  const conversationItemBase = isManager ? '/api/ai/manager-conversations' : '/api/ai/conversations';
  ...
  // sits at /manager/cola, seller at /s/<slug>/cola.
  const colaBaseUrl = isManager ? '/manager' : `/s/${slug}/cola`;

  function handleSelectConversation(conv: Conversation) {
    setDrawerOpen(false);
    if (conv.id === activeConversationId) return;
    startConversationTransition(() => {
      router.push(`${colaBaseUrl}?conversationId=${conv.id}`, { scroll: false });
    });
  }

  async function handleNewConversation() {
    const endpoint = isManager ? '/api/ai/manager-conversations' : '/api/ai/conversations';
    const body = isManager ? {} : { slug };
    try {
  ...
  // call is delayed enough that the seller can walk it back without a
  ...
  useEffect(() => {
    return () => {
      ...
      // a page nav after the seller walked away from the toast.
    ...
    // conversationItemBase is constant per mount (isManager never changes), so
  ...
  async function handleDeleteConversation(id: string) {
    ...
    // Optimistic: remove from the sidebar immediately so the seller sees
    // the action took. Navigate away from the transcript if it was active.
    setConversations((prev) => prev.filter((c) => c.id !== id));
    if (wasActive) {
      startConversationTransition(() => {
        router.push(colaBaseUrl, { scroll: false });
      });
    }

    // Manager conversations live in their own "ManagerConversation" table, so
    // the delete routes to the manager-gated /api/ai/manager-conversations/<id>
    // (which scopes the delete to the caller's company). The seller route
    // is used for the seller variant.
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          // seller. They've already moved on by now, so the toast carries
    ...
    toast.success('Conversation deleted.', {
      ...
      action: {
        ...
        onClick: () => {
          ...
          if (wasActive) {
            startConversationTransition(() => {
              router.push(`${colaBaseUrl}?conversationId=${id}`, { scroll: false });
            });
          }
        },
      },
  ...
  const handleMentionSearch = useCallback(
    async (query: string): Promise<MentionItem[]> => {
      ...
      try {
        ...
        // id; once a search succeeds again the seller just dismisses it.
  ...
  const handleSend = useCallback(
    async (
      ...
      // clean — the helper no-ops when the seller hasn't enabled sound
  ...
  // (the server doesn't know the seller's local hour); we render an
  ...
  // "or just tell Cola →" shortcuts on /contacts and /deals, and
  ...
  //
  // Manager variant doesn't surface drafts/questions on the chat home —
  // those endpoints are space-scoped (seller-only). Skip the fetch.
  ...
  // Day-one signal: zero of everything. The seller has truly never engaged.
  ...
  const TOOL_ACTION_MAP: Record<string, string> = {
    ...
    schedule_demo: 'Checking the calendar…',
    reschedule_demo: 'Checking the calendar…',
    find_demos: 'Checking the calendar…',
  ...
  const TOOL_STEP_KEYWORDS: Record<string, string[]> = {
    ...
    schedule_demo: ['demo', 'calendar', 'schedule'],
    reschedule_demo: ['demo', 'calendar', 'schedule'],
    find_demos: ['demo', 'calendar'],
    set_followup: ['follow'],
  ...
  const currentAction = useMemo<string | null>(() => {
    ...
    if (liveCallIds && liveCallIds.size > 0) {
      for (const block of tailMessage.blocks) {
        if (
          ...
          // get a friendly verb derived from the toolkit name so sellers
    ...
    // dead air with a single calm status so the seller knows Cola is on
    // it, not stuck.
  ...
  // inside ColaPromptBox itself.
  const renderInput = () => (
    <div>
      <ColaPromptBox
        placeholder="Message Cola, or press / for skills…"
        onSend={handleSend}
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1.5">
              ...
              <p className="text-sm text-muted-foreground">
                Tune what Cola does on its own and what it brings to you.
              </p>
            </div>
          </div>
    ...
    <div className="relative flex flex-col h-full min-h-0">
      ...
      <div className="absolute top-1.5 right-2 sm:top-2 sm:right-3 z-20 flex items-center gap-1.5">
        ...
        {/* One three-dots menu — folds New chat, History, Brief/Drafts, and
            (seller) Run now / Memory / Cola settings into a single animated
            ...
            motion-reduce aware). Routes resolve per variant — seller:
            /s/<slug>/cola/*, manager: /manager/* — never a broken /s//… href. */}
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" sideOffset={6} className="w-48">
            ...
            <DropdownMenuItem asChild>
              <Link
                href={isManager ? '/manager/brief' : `/s/${slug}/cola/brief`}
            ...
            <DropdownMenuItem asChild>
              <Link
                href={isManager ? '/manager/reviews' : `/s/${slug}/cola/inbox`}
                className="cursor-pointer"
              >
            ...
            {!isManager && (
              <>
                ...
                <DropdownMenuItem asChild>
                  <Link href={`/s/${slug}/cola/memory`} className="cursor-pointer">
                    <NotebookText size={14} className="mr-2" />
                    Memory
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link href={`/s/${slug}/cola?tab=settings`} className="cursor-pointer">
        ...
        {/* Split panel disabled on manager variant — RightPanel tabs
            (contacts/deals/etc.) are seller-scoped in Phase 1. */}
        {!isManager && <SplitPanelToggle isSplit={isSplit} onToggle={toggleSplit} />}
      </div>
      ...
      {isLoadingConversation ? (
        /* Skeleton mirrors the empty-state hero shape below — a centered
           ...
           the bottom. Same shapes as app/s/[slug]/cola/loading.tsx so the
        ...
        /* Empty → active swap.
           ...
           `layoutId="cola-composer"` to morph the composer between hero
           and docked positions, but popLayout keeps the exiting node alive
           through the exit animation, so the seller briefly had TWO
           ...
           and the conversation enter — reads as "Cola heard you, getting
        ...
        <AnimatePresence mode="wait" initial={false}>
          {isEmpty && (
            ...
            >
              {/* The daily brief lives at /cola/brief (sidebar entry).
              ...
              <div className="w-full max-w-2xl">
                <motion.h1
                  ...
                  {/* Day-one guidance — seller only, and only for users with
                      zero history. The manager chat goes straight from greeting
                      to composer (no subtext) per the company design. */}
                  {!isManager && isFresh && (
                    <motion.p
              ...
              <div className="w-full max-w-3xl mx-auto chat-content-wrap pt-12 sm:pt-14 pb-36">
                ...
                <div className="space-y-7">
                  {messages.map((msg, i) => {
                    ...
                    // — typed text reading "instantly there" is the seller's
                    // own action, not Cola delivering something.
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 pt-0.5 space-y-3">
                            ...
                            {/* Inline retry button — shown on the tail error
                                message so the seller doesn't have to retype. */}
                  ...
                  {/* Errors land inline as Cola assistant messages
                      (see useAgentTask.landColaError) so the failure mode
                      reads like Cola talking, not a red system banner. The
          ...
          {/* Docked input — pinned to the bottom of the active thread.
              Previously a `motion.div` with `layoutId="cola-composer"`
