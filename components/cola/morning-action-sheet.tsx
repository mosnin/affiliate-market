 ...
 * when the seller taps a compose action. Replaces the chat-prefill
 * teleporter — Phase 7. The seller never leaves the home.
 *
...
interface Props {
  ...
  onSent: () => void;
  /** Called when the seller taps Cancel. */
  onCancel: () => void;
}
...
export function MorningActionSheet({ slug, intent, context, onSent, onCancel }: Props) {
  ...
  // reopening the panel re-drafts (the seller can re-roll by re-tapping).
  useEffect(() => {
  ...
  async function handleSend() {
    ...
    try {
      ...
      // The "not_configured" path is a half-win — the seller's intent was
  ...
  function handleEdit() {
    if (!draft) return;
    // The escape hatch — drop the seller into the chat with the draft as
    ...
    const prefill = draft.subject
      ...
      : draft.body;
    router.push(`/s/${slug}/cola?prefill=${encodeURIComponent(prefill)}`);
  }
  ...
  return (
    ...
    >
      ...
      {phase === 'error' && (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => router.push(`/s/${slug}/cola`)}
              className="inline-flex h-9 items-center rounded-full border border-border/70 bg-background px-4 text-sm hover:bg-muted/40"
            >
              Open chat
            </button>
      ...
      {/* The win moment — the same surface that fired the send transforms in
          place into one calm sentence in Cola's voice. The component owns
