...
/**
 * PostDemoRecorder — the one client component on /cola/log.
 *
 * State machine:
 ...
 *     Tap is forgiving.
 *   - The transcript is invisible. The seller never sees it. They see
 *     the actions land, which is the substance.
 *   - "Approve all" is the primary path. Editing exists but is one click
 *     away — the brief says most sellers approve all.
...
interface Proposal {
  ...
  summary: string;
  /** Server-resolved Cola-voice line that uses real names instead of IDs.
...
interface Props {
   ...
   *  toward this person. Sent through as `contextHint` on the post-demo
   *  request body. The recorder UI is unchanged; the seller never sees
   *  the id. */
  personId?: string;
...
const MAX_RECORDING_MS = 5 * 60 * 1000; // 5 minutes — Whisper can take more, but a demo debrief shouldn't.

export function PostDemoRecorder({ slug, personId, dealId }: Props) {
  ...
  // The seller never sees these — they're a hint to the proposal model so
  ...
  const processAudio = useCallback(async (blob: Blob) => {
    ...
    try {
      ...
      const res = await fetch('/api/cola/transcribe', { method: 'POST', body: fd });
    ...
    setState('processing');
    try {
      const res = await fetch('/api/cola/post-demo', {
        method: 'POST',
  ...
  const handleApproveAll = useCallback(async () => {
    ...
    setState('approving');
    try {
      const res = await fetch('/api/cola/post-demo/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
/** Keys we never expose to the seller in the args editor — they're
...
const HIDDEN_ARG_KEYS = new Set(['personId', 'dealId', 'contactId', 'productId', 'demoId']);
...
function ArgsEditor({
  ...
  // synthesize new keys — the model already chose them; the seller edits
