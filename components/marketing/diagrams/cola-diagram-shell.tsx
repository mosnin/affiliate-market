...
/**
 * `<ColaDiagramShell>` — the paper-flat container every animated UI
...
interface ColaDiagramMotionContextValue {
...
const ColaDiagramMotionContext = createContext<ColaDiagramMotionContextValue>({
  reduced: false,
});
 ...
 */
export function useDiagramMotion(): ColaDiagramMotionContextValue {
  return useContext(ColaDiagramMotionContext);
}

export interface ColaDiagramShellProps {
  aspect?: Aspect;
...
export function ColaDiagramShell({
  aspect = 'video',
  pad = 8,
  caption,
  className,
  children,
}: ColaDiagramShellProps) {
  const reduced = useReducedMotion() ?? false;
  const padClass = pad === 6 ? 'p-6' : 'p-8';

  return (
    <ColaDiagramMotionContext.Provider value={{ reduced }}>
      <div
        data-cola-diagram
        className={cn(
    ...
    </ColaDiagramMotionContext.Provider>
  );
}

/**
 * The Cola authorship badge as it lives inside diagrams — same
 ...
 * names; this is one of them (AGENT_BADGE / COLA_AVATAR).
 */
export function DiagramColaBadge({
  label = 'Cola',
  className,
}: {
  label?: string;
