...
interface DemoPrepCardProps {
  demoId: string;
}

export function DemoPrepCard({ demoId }: DemoPrepCardProps) {
  const [prep, setPrep] = useState<DemoPrepData | null>(null);
  ...
  async function loadPrep() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/demos/${demoId}/prep`);
      if (res.ok) {
        ...
        <div className="absolute right-0 top-full mt-2 z-30 w-80 sm:w-96 rounded-xl border border-border/70 bg-card shadow-[0_4px_24px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)] overflow-hidden">
          ...
          <div className="px-4 py-3 bg-primary/5 border-b border-border">
            <div className="flex items-center gap-2 text-xs font-semibold text-primary">
              ...
              Demo Prep — {prep.guestName}
            </div>
            <div className="flex items-center gap-3 mt-1.5 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-1"><CalendarDays size={10} /> {prep.demoDate}</span>
              <span className="flex items-center gap-1"><Clock size={10} /> {prep.demoTime} ({prep.duration}min)</span>
            </div>
          </div>
          ...
          <div className="p-4 space-y-3 max-h-80 overflow-y-auto">
            ...
            <div className="space-y-1">
              ...
              <div className="space-y-0.5 text-xs">
                ...
                {prep.productAddress && <p className="flex items-center gap-1.5"><MapPin size={11} className="text-muted-foreground" /> {prep.productAddress}</p>}
                {prep.previousDemos > 0 && <p className="text-[10px] text-muted-foreground">{prep.previousDemos} previous demo{prep.previousDemos > 1 ? 's' : ''}</p>}
              </div>
            </div>
