...
interface PostDemoFeedbackProps {
  token: string;
  guestName: string;
  businessName: string;
}

export function PostDemoFeedback({ token, guestName, businessName }: PostDemoFeedbackProps) {
  ...
  async function submitFeedback() {
    ...
    setError('');
    try {
      const res = await fetch('/api/demos/feedback', {
        method: 'POST',
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6 space-y-4">
      <div className="text-center space-y-1">
        <h3 className="text-lg font-semibold">How was your demo?</h3>
