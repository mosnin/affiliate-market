...
/**
 * Manager member profile form — an owner/admin's own profile within the
 * company. Mirror of the seller profile-section (display name, title, bio,
 * photo, phone) but persisted per-member on CompanyMembership via
 * /api/manager/profile (self-scoped). Photo uploads use /api/upload/onboarding
 * (works for manager-only accounts with no Space).
 */
    ...
    <form onSubmit={handleSave} className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0">
          <button
            type="button"
            onClick={() => document.getElementById('manager-profile-photo')?.click()}
      ...
      <div className="space-y-1.5">
        <Label htmlFor="manager-display-name" className="text-[12.5px] font-medium text-foreground">
          Display name
        </Label>
        <Input
      ...
      <div className="space-y-1.5">
        <Label htmlFor="manager-title" className="text-[12.5px] font-medium text-foreground">
          Title
        </Label>
        <Input
          id="manager-title"
          ...
          placeholder="Managing Manager"
        />
      </div>

      <div className="space-y-1.5">
        <Label htmlFor="manager-phone" className="text-[12.5px] font-medium text-foreground">
          Phone number
        </Label>
        <Input
          id="manager-phone"
      ...
      <div className="space-y-1.5">
        <Label htmlFor="manager-bio" className="text-[12.5px] font-medium text-foreground">
          Bio
        </Label>
        <Textarea
          id="manager-bio"
          ...
          placeholder="A short bio shown alongside your name in the company."
          rows={3}
        />
      </div>
