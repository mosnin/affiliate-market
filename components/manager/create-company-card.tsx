...
interface CreateCompanyCardProps {
  /** If the user is already a manager, link to their dashboard instead. */
  existingCompanyName?: string | null;
}

export function CreateCompanyCard({ existingCompanyName }: CreateCompanyCardProps) {
  const [name, setName] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  // Already a manager — show link
    ...
    <Card>
      <CardContent className="px-5 py-4 space-y-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold">Create a company</p>
            <p className="text-xs text-muted-foreground">Invite sellers and get team visibility</p>
          </div>
        </div>
