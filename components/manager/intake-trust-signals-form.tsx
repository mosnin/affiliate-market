...
/**
 * Company-level intake trust signals — set once by the company admin
 * and inherited by every agent's /apply/b/[companyId] intake footer.
 * If both per-space and per-company values exist, the company value
 * wins on the company variant.
 */
export function CompanyIntakeTrustSignalsForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/manager/settings', {
        ...
        body: JSON.stringify({
          companyLicenseNumber: licenseNumber.trim() || null,
          companyFairHousingNotice: fairHousingNotice || null,
          companyShowEqualHousingMark: showEqualHousingMark,
        }),
      });
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="space-y-1.5">
        ...
        <Input
          ...
          disabled={!isOwner}
        />
        <p className={CAPTION}>Your company license number, shown verbatim.</p>
      </div>

      <div className="space-y-1.5">
        <Label htmlFor="companyFairHousingNotice">Fair Housing notice</Label>
        <Textarea
          id="companyFairHousingNotice"
      ...
      <div className="flex items-start gap-3 pt-1">
        <Switch
          id="companyShowEqualHousingMark"
        ...
        <div className="space-y-0.5">
          <Label
            htmlFor="companyShowEqualHousingMark"
      ...
      {!isOwner && (
        <p className={BODY_MUTED}>
          Only the company owner or admins can edit these settings.
        </p>
      )}
    </form>
  );
