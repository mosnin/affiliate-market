...
export function OffboardMemberDialog({
  ...
  async function callOffboard<T>(dryRun: boolean): Promise<T> {
    const res = await fetch(`/api/manager/members/${member.id}/offboard`, {
      method: 'POST',
        ...
        {state === 'choose' && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Pick which active member should receive this agent&apos;s company
                contacts, deals, and open demos. This is reversible manually but not
          ...
          <>
            ...
            {nothingToTransfer ? (
              <div className="rounded-lg border border-border bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
                Nothing to transfer &mdash; this agent has no company-owned data.
              </div>
            ) : (
              <div className="rounded-lg border border-border bg-card p-4 space-y-2">
                <p className="text-sm">
                  <span className="font-semibold tabular-nums">{preview.contactCount}</span>{' '}
                  company {preview.contactCount === 1 ? 'contact' : 'contacts'} will move
                  to{' '}
                ...
                <p className="text-sm">
                  <span className="font-semibold tabular-nums">{preview.openDemoCount}</span>{' '}
                  open {preview.openDemoCount === 1 ? 'demo' : 'demos'} will be reassigned
                </p>
              </div>
            )}
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {result.demosMoved === 1 ? 'demo' : 'demos'} moved to{' '}
