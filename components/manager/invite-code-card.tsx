...
export function InviteCodeCard({ isOwner = true }: { isOwner?: boolean }) {
  ...
  async function handleGenerate() {
    setGenerating(true);
    try {
      const res = await fetch('/api/manager/join-code', { method: 'POST' });
      ...
      <CardContent className="px-5 py-4 space-y-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Share this code — anyone can use it to join as a Seller</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-xs text-muted-foreground">No invite code set. Ask the company owner to generate one.</p>
        )}
