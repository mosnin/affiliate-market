...
import type { ManagerMorningResponse } from '@/app/api/manager/morning/route';

/**
 * The /manager home's chief-of-staff opening line.
 *
 * Replaces the old snapshot-counts pattern ("5 sellers, 2 drafts, 3 hot
 * leads") with ONE prioritized action the manager should take RIGHT NOW —
 * composed server-side by /api/manager/morning. The single sentence is the
 ...
 * deep-links into /manager/cola with the prompt prefilled — making the
...
const STATS_LOADING: ManagerMorningResponse['stats'] = {
  sellers: 0,
  activeDeals: 0,
  gciMtd: 0,
};
 ...
 * manager's eye should land on. Falls back to rendering the whole headline
...
export function ManagerMorningStory() {
  const router = useRouter();
  const [data, setData] = useState<ManagerMorningResponse | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch('/api/manager/morning', { signal: controller.signal });
        if (res.ok) {
          const body = (await res.json()) as ManagerMorningResponse;
          setData(body);
        } else {
          setError(true);
        }
  ...
  function handleTap() {
    ...
    router.push(
      `/manager/cola?prefill=${encodeURIComponent(suggestedPrompt)}`,
    );
  }
...
/**
 * Hairline-divider context row. Three numbers — sellers, active deals,
 * GCI MTD — rendered as small, muted facts beneath the headline. This is
 * intentionally NOT the snapshot strip from app/manager/page.tsx (that's
