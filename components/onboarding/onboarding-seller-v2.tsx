...
/**
 * Seller onboarding - V2, the conversational flow.
 *
 * The seller doesn't fill out a form; they have a conversation with Cola,
 ...
 *                     the Cola mark, then a blur-out into the chat.
 *   Act II (chat)   - Cola types each question; the seller answers through
 *                     real inline inputs rendered as their own chat bubbles.
 *                     Same surface, same motion as the live intake chat.
 *   Act III (ready) - Cola types a real first-touch draft (the payoff), then
 ...
 * call (check_slug, save_profile, create_space, save_seller_profile,
 * complete) and the manager-create/space-patch sequence are byte-for-byte what
 ...
 * (`onboarding-seller.tsx`) stays the rollback path - don't refactor it.
 */
...
import { ColaSays, UserSays, AnswerAffordance, Thread } from './onboarding-chat';
...
const ROLE_LABEL: Record<Role, string> = {
  ...
  company_owner: 'I own the company',
};

export function OnboardingSellerV2({ defaultName }: Props) {
  ...
  // Greet has no input - once Cola finishes saying hello, glide to the
  ...
  const handleCreateWorkspace = useCallback(async () => {
    ...
    try {
      ...
      await fetch('/api/onboarding', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'save_seller_profile', role, zipCode, yearsExperience: TENURE_TO_YEARS[tenure] }),
      });
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      ...
      if (role === 'company_owner') {
        ...
        const managerData = (await managerRes.json().catch(() => ({}))) as { company?: { id?: string } };
      ...
      redirectRef.current = `/s/${slug}/cola`;
      setPhase('ready');
    } catch {
  ...
  const promptFor = useCallback((s: Step): string => {
    switch (s) {
      case 'greet': return "I'm Cola, your new companion. From here on, I'm working right alongside you.";
        ...
        <div className="relative z-[1]">
          <Thread signal={`${stepIdx}:${typedDone}`}>
            ...
            {ORDER.slice(0, stepIdx).map((id) => {
              ...
              return (
                <Fragment key={id}>
                  <ColaSays text={promptFor(id)} />
                  {answer && <UserSays>{answer}</UserSays>}
                </Fragment>
              );
            })}

            {/* Active turn - Cola types, then the answer affordance fades in. */}
            <ColaSays
              key={`active-${current}`}
      ...
      case 'promise':
        return (
          <div className="flex justify-end">
            <button type="button" onClick={advance} className={COLA_PILL}>
