...
/**
 * Seller onboarding — six stages, ~4 minutes.
 *
 * Replaces the one-screen `OnboardingQuick`. The point isn't to "collect more
 * fields" — it's that by the end, Cola already knows the seller's role,
 ...
 *   6. Plan                — Cola's first-week plan, derived from the above.
 *
 * Persistence:
 *   - End of stage 2: `save_profile` + `create_space`. Slug is committed.
 *   - End of stages 3, 4, 5: `save_seller_profile` upsert (cumulative).
 *   - End of stage 6: `complete` → redirect to /s/{slug}/cola.
 *
 * Refresh recovery is intentionally minimal in Phase 1 — if the seller
...
export function OnboardingSeller({ defaultName }: Props) {
  ...
  // Auto-derive slug from business name until the seller edits it.
  useEffect(() => {
  ...
  // offering "company_owner" as a role but then completing as `accountType:
  // 'seller'` and routing to /s/{slug}/cola — silently dropping the user
  // who said they ran a company onto the solo-seller dashboard, with no
  // Company row ever created. Now: when role is company_owner, complete
  // as `accountType: 'both'`, call /api/manager/create with the workspace
  // business name, link the new Space to the new Company, and route to
  // /manager. Mirrors the manager-with-workspace branch in onboarding-flow.tsx.
  const handleFinish = useCallback(async () => {
    ...
    try {
      ...
      if (role === 'company_owner') {
        ...
        if (!managerRes.ok) throw new Error('manager_create');
        const managerData = (await managerRes.json().catch(() => ({}))) as {
          company?: { id?: string };
        };
        const newCompanyId = managerData.company?.id;
        if (newCompanyId) {
          // Best-effort link — failure here doesn't block the redirect; the
          // company exists and the user is a manager_owner either way.
          await fetch('/api/spaces', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ slug, companyId: newCompanyId }),
          }).catch(() => undefined);
        }

        toast.success("Company created. Cola is ready.");
        router.push('/manager');
        return;
      }
      ...
      const completeRes = await fetch('/api/onboarding', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'complete', accountType: 'seller' }),
      });
      if (!completeRes.ok) throw new Error('complete');

      toast.success("You're in. Cola is ready.");
      router.push(`/s/${slug}/cola`);
    } catch {
    ...
    <div className="space-y-8">
      ...
      <Section label="What are you running?">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          <PickerButton selected={props.role === 'company_owner'} onClick={() => props.onChangeRole('company_owner')}>
            <span className="font-medium">I own the company</span>
          </PickerButton>
        </div>
      </Section>
...
function StageVoice({
  ...
  // Same scenario for both — only tone differs. Seller picks the one that
  // sounds like them; Cola pins that as the default tone going forward.
  ...
  const direct = `Hi — yes, 1422 Pine is available. Asking $625K, 3 bed / 2 bath, last open house had 11 groups through. I can hold a private demo Tue 5–7p or Wed 6–7p. Which works? — ${firstName}, ${business}`;
