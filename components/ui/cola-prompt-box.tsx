...
/** A skill the seller can pick from the `/` menu. */
export interface SkillItem {
  slug: string;
  title: string;
  description: string;
  /** Composer text; may carry one {placeholder} for the seller to fill. */
  prompt: string;
}
 ...
 * the seller types straight over it; with no token the cursor lands at the
...
/** Per-skill menu icons, keyed by slug. Falls back to the Cola mark. */
...
/**
 * Chat vs Agent — the per-message runtime the seller picks in the composer.
 *   - 'chat'  → fast, cheap answer. One model call + read-only search over
 *               their data. Finds and explains; never acts.
 *   - 'agent' → Cola can act: create, send, schedule, run integrations.
...
interface ColaPromptBoxProps {
   ...
   * day-one welcome to seed "Hi Cola, my most recent lead is …" without
  ...
  skills?: SkillItem[];
  /**
   * Show the Chat ↔ Agent runtime switch. Default true (seller surface).
   * The manager chat is always its own agent, so the switch would be a lie
...
export const ColaPromptBox = React.forwardRef<HTMLTextAreaElement, ColaPromptBoxProps>(
  function ColaPromptBox(
    {
      placeholder = 'Message Cola…',
      onSend,
      onMentionSearch,
      onAttach,
      onVoiceStart,
    ...
    // text-keyed so the seller can edit/clear without us re-stomping it.
    ...
    // textarea keep it open — the seller is still typing the query.
    useEffect(() => {
      if (!slashOpen) return;
      const onDown = (e: MouseEvent) => {
    ...
    function handleSubmit() {
      ...
      // next message doesn't silently run the tool loop the seller forgot
      // they'd left on.
      setChatMode('chat');
                    ...
                    <Tooltip key={m}>
                      ...
                      <TooltipContent side="top" sideOffset={6}>
                        {m === 'chat'
                          ...
                          : 'Cola can act — create, send, schedule'}
                      </TooltipContent>
                    </Tooltip>
                  );
                })}
