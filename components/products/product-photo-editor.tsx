...
/**
 * ProductPhotoEditor — multi-image upload + featured-photo picker.
 *
 ...
 * featured photo. The list page reads `product.photos[0]`. The detail hero
 * reads `product.photos[0]`. So "set as featured" here is just "move this
 * URL to index 0" — no separate flag, no `featuredPhotoIndex` column, no
 * `sortOrder` to keep consistent. The array order IS the order.
 *
 * Storage: posts to `/api/upload` with `type=product-photo`, which writes
 * to the `product-photos/{spaceId}/` prefix the storage-gc sweeper already
 * scans against `Product.photos`. No new migration, no new endpoint, no
...
export function ProductPhotoEditor({ value, onChange, max = DEFAULT_MAX }: Props) {
  const fileRef = useRef<HTMLInputElement>(null);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState('');
  // URL of the tile the seller just promoted to position 0 — drives a brief
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <motion.img
                      src={url}
                      alt={isFeatured ? 'Featured product photo' : `Product photo ${idx + 1}`}
