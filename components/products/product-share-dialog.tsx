...
import type { ProductPacket } from '@/lib/types';
import type { DealDocument } from '@/lib/deals/documents';
import { documentKindLabel } from '@/lib/deals/documents';

interface Props {
  productId: string;
  /** Deal ids linked to this product — used to list their documents. */
...
/**
 * Dialog for creating / managing listing-packet share links for a product.
 * Fetches existing packets + candidate documents (from deals linked to this
 * product), lets the seller curate which documents go in, and hands back
 * the shareable URL on create.
 */
export function ProductShareDialog({ productId, linkedDealIds, origin, onClose }: Props) {
  const [packets, setPackets] = useState<ProductPacket[]>([]);
    ...
    async function load() {
      ...
      try {
        const [packetsRes, ...docResults] = await Promise.all([
          fetch(`/api/products/${productId}/packets`),
  ...
  async function create() {
    ...
    setCreating(true);
    try {
      const res = await fetch(`/api/products/${productId}/packets`, {
        method: 'POST',
      ...
      const packet: ProductPacket = await res.json();
  ...
  async function revoke(p: ProductPacket) {
    if (!confirm("Revoke this link? Anyone with the URL loses access right away.")) return;
    const res = await fetch(`/api/products/${productId}/packets/${p.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function copyLink(p: ProductPacket) {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 backdrop-blur-sm pt-[8vh] px-4" onClick={onClose}>
      <div className="w-full max-w-[720px] rounded-xl border border-border/70 bg-card shadow-[0_4px_24px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-3.5 border-b border-border flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">Share this product</h2>
          <button
            type="button"
            onClick={onClose}
