...
import type { DemosAnalyticsData } from '@/lib/analytics-data';
import {
  SECTION_RHYTHM,
  STAT_NUMBER,
  TITLE_FONT,
...
const demosOverTimeConfig = {
  count: { label: 'Demos', color: 'hsl(var(--foreground))' },
} satisfies ChartConfig;

const demosByStatusConfig = {
...
export function DemosView({ data }: { data: DemosAnalyticsData }) {
  return (
    <div className={SECTION_RHYTHM}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-border/70 rounded-xl overflow-hidden border border-border/70">
        <StatCell label="Total demos" value={data.totalDemos} sub="all time" />
        <StatCell label="Completed" value={data.completedDemos} sub="demos finished" />
        <StatCell
          label="Cancelled / No-show"
          value={data.cancelledDemos + data.noShowDemos}
          sub={`${data.noShowDemos} no-show`}
        />
        <StatCell
          label="Demo-to-deal rate"
          value={data.completedDemos > 0 ? `${data.demoConversionRate}%` : '--'}
          sub={`${data.demosConvertedToDeals} converted`}
        />
      </div>

      {/* Charts */}
      <div className="grid sm:grid-cols-2 gap-4">
        <ChartSection title="Demos over time" sub="Demo bookings per month">
          <ChartContainer config={demosOverTimeConfig} className="h-[220px] w-full">
            <AreaChart data={data.demosOverTime}>
              <defs>
                <linearGradient id="demosGrad" x1="0" y1="0" x2="0" y2="1">
        ...
        <ChartSection title="Demos by status" sub="Breakdown of demo outcomes">
          {data.demosByStatus.length > 0 ? (
            <ChartContainer config={demosByStatusConfig} className="h-[220px] w-full">
              <PieChart>
                <Pie
                  data={data.demosByStatus}
                  dataKey="count"
                  nameKey="label"
                  cx="50%"
                  cy="50%"
                  ...
                  strokeWidth={2}
                >
                  {data.demosByStatus.map((entry, i) => (
                    <Cell
                      key={entry.label}
      ...
      {/* Conversion funnel */}
      <ChartSection title="Demo conversion funnel" sub="From booked demos to closed deals">
        <div className="flex flex-col sm:flex-row gap-4 items-stretch py-2">
          {[
            { label: 'Booked', count: data.totalDemos },
            { label: 'Completed', count: data.completedDemos },
            { label: 'Converted to deal', count: data.demosConvertedToDeals },
          ].map((stage, i) => {
            const opacity = 1 - i * 0.15;
            const pct =
              data.totalDemos > 0 && i > 0
                ? Math.round((stage.count / data.totalDemos) * 100)
                : null;
            return (
              <div
                key={stage.label}
      ...
      {data.totalDemos === 0 && (
        <div className="rounded-xl border border-border/70 bg-background px-6 py-12 text-center">
          <p className={BODY_MUTED}>
            Demo analytics will appear here once demos are scheduled and completed.
          </p>
        </div>
      )}
    </div>
