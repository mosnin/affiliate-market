...
/**
 * Account-switch transition — the Cola-logo swipe shown when a user moves
 * between their seller workspace and their company.
 *
 * Why a sessionStorage flag instead of an in-place overlay: switching crosses
 * a layout boundary (`/s/[slug]/*` ↔ `/manager/*`), so any overlay rendered by
 ...
 *   peekSwitchFlag()        — read without clearing (lets ColaSplash yield
...
const SWITCH_FLAG = 'cola:account-switch';
...
/** Read the flag WITHOUT clearing it. ColaSplash uses this (in a render-time
...
/**
 * The swipe itself. A full-bleed panel carries the Cola mark in from the
...
export function AccountSwitchSwipe() {
  ...
  // any effect runs (keeps it in lockstep with ColaSplash's render-time read).
