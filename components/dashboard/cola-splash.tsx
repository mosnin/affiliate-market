...
/**
 * Opening moment. On every app/PWA open, Cola greets the seller by name
 ...
 * they were away, signs off with the small Cola mark, then the whole thing
...
// bundle: it survives client-side navigation (so switching between the seller
// and manager dashboards does NOT replay the heavy splash) but resets on a full
// page reload (so a first open or hard refresh plays it again). The lighter
// Cola-logo account-switch swipe is a separate component and is unaffected.
let splashPlayed = false;

export interface ColaSnapshot {
  newLeads: number;
  followUpsDue: number;
  draftsReady: number;
}
...
function buildItems(s: ColaSnapshot): { n: number; label: string }[] {
...
export function ColaSplash({
  ...
  // hands the moment to the lighter Cola-logo swipe instead.
          ...
          <AnimatePresence mode="wait">
            {stage === "greeting" ? (
              ...
              >
                ...
                {/* Small Cola mark — the sign-off the overlay swipes up with. */}
                <motion.div
