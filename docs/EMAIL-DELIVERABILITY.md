...
Transactional mail Cola sends from its **own** domain (follow-up digests,
manager weekly reports, system notifications via Resend) needs DNS
...
> the sending domain (wherever `usecola.com` DNS is hosted), **not** in this
...
1. **User-connected Gmail / Outlook sending (no action needed).**
   When a seller connects their own Gmail/Outlook (via the integrations layer)
...
2. **Transactional mail from the app's own domain (records REQUIRED).**
   ...
   `RESEND_FROM_EMAIL`, default sender `notifications@alerts.usecola.com`) —
   comes from a Cola-owned domain. That domain needs SPF, DKIM, and DMARC, or
...
Assume the sending subdomain is **`alerts.usecola.com`** (matches the Resend
default `notifications@alerts.usecola.com`). Adjust the host to whatever
...
```
Type:  TXT
Host:  alerts.usecola.com        (or "send.usecola.com" per Resend)
...
Host:  <selector>._domainkey.alerts.usecola.com
...
```
Type:  TXT
Host:  _dmarc.usecola.com
Value: v=DMARC1; p=none; rua=mailto:dmarc@usecola.com; fo=1; adkim=s; aspf=s
```
...
2. **Command line spot-check:**
   ```
   dig +short TXT alerts.usecola.com          # SPF present
   dig +short CNAME <selector>._domainkey.alerts.usecola.com   # DKIM present
   dig +short TXT _dmarc.usecola.com          # DMARC present
   ```
