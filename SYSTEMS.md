...
| Demo booking, double-booking, availability, calendar sync | **4. Demos & Calendar** |
| Cola chat in-app: replies, tool calls, approval prompts, streaming | **5. Cola Chat Runtime (TS)** |
| Cola acting on its own, didn't fire on an event, cron, swarm, runaway spend | **6. Autonomous Agent** |
| Manager dashboard, team roster, lead routing, commissions, reviews, invites, seats | **7. Company Tier** |
| Applicant/client portal login, messaging a seller, client documents | **8. Client Portal** |
...
- **Entry:** `middleware.ts` (Clerk gate, ban/offboarding fast-path, admin pre-check); `lib/api-auth.ts` (`requireAuth`, `requireSpaceOwner`, `requireContactAccess` — the canonical API choke-point); `lib/permissions.ts` (`requireManager`/`requirePlatformAdmin`/`getManagerContext`); `app/auth/redirect/page.tsx` (post-login routing).
- **Owns:** `User` (`clerkId`, `status`, `platformRole`), `CompanyMembership`, `Space.ownerId`.
...
- **Entry:** `app/api/public/apply/route.ts`, `app/api/public/apply/company/route.ts`, `app/api/public/intake-chat/route.ts`, `app/apply/[slug]/**`; scoring: `lib/lead-scoring.ts` → `lib/scoring/engine.ts` (deterministic) + `lib/scoring/enhance.ts` (optional LLM summary, `gpt-4.1-mini`).
- **Owns:** `Contact`, `ApplicationStatusUpdate`, `FormDraft`; reads `Space`, `SpaceSetting`, `Company`.
...
- **Fragile seams:** (1) Parent rows enforce the space boundary, but several **child-table queries** (`DealActivity`, `Demo` in timeline, rescore updates) rely on FK correctness instead of their own `.eq('spaceId')`. (2) Reorder is atomic via the `reorder_deal` RPC; the checklist-shift loop is not. (3) Pipeline bootstrap is read-then-multi-write with no transaction (concurrent first-load can duplicate).

## 4. Demos & Calendar

Public demo booking, guest self-management by token, agent demo CRUD, Google Calendar sync.

- **Entry:** `app/api/demos/book/route.ts`, `app/api/demos/available/route.ts`, `app/api/demos/[id]/route.ts`, `app/api/demos/gcal/route.ts`, `app/api/calendar/events/route.ts`, `app/demo/[token]/page.tsx`, `app/book/[slug]/**`; `lib/gcal-helpers.ts`, `lib/demo-emails.ts`.
- **Owns:** `Demo`, `DemoFeedback`, `DemoWaitlist`, `DemoAvailabilityOverride`, `DemoProductProfile`, `GoogleCalendarToken`, `CalendarEventMirror`.
- **External:** Google Calendar OAuth2 + freeBusy, Composio (agent calendar path), Resend/SMS.
- **Fragile seams:** (1) The `book_demo_atomic` RPC is the *sole* double-booking guard — any path bypassing it reintroduces the race. (2) `GoogleCalendarToken.accessToken` encryption is inconsistent across three refresh paths (one reads/writes plaintext). (3) Availability conflict can scope to one product, missing same-seller cross-product overlaps.

## 5. Cola Chat Runtime (TS, on-demand)

The default in-app chat backend: `@openai/agents` loop, 55 domain tools, SSE streaming, approval-gated mutations. `COLA_CHAT_RUNTIME=ts` (default); `=modal` proxies to the sandbox.
...
Scheduled + event-driven Cola runs per workspace, plus multi-agent swarms, in a Python/Modal sandbox.
...
Multi-agent org oversight: membership, lead routing, commissions, deal reviews, templates, invitations, seats. Manager↔seller isolation is the recurring bug class (PRs #294–#296).

- **Entry:** `lib/permissions.ts` (`requireManager`/`getManagerContext`/`getManagerMemberContext`), `lib/company-routing.ts`, `lib/company-seats.ts`, `app/api/manager/**`, `app/api/invitations/[token]/route.ts`.
- **Owns:** `Company`, `CompanyMembership`, `Invitation`, `CompanyRemoval`, `CommissionLedger`, `DealRoutingRule`, `CompanyTemplate`, `DealReviewRequest/Comment`, `ManagerNotification`.
- **Fragile seams:** (1) A manager route must verify the caller belongs to the **specific** company that owns the resource, not just "is a manager." (2) `getManagerContext` multi-membership tie-break is order-dependent. (3) Seat-cap + pending-invite count has no DB-level serialization (double-spend race); invitation accept re-activates offboarded users by design.
...
AI content generation, file/document storage, product packets, e-sign, third-party toolkits.
...
- **Owns:** `File`, `StudioGeneration/Brand/Post`, `ContactDocument`, `DealDocument`, `ProductPacket`, `SignatureRequest`, `IntegrationConnection`.
...
- **Owns:** `Space.stripe*`/`trialUsedAt`, `Company.stripe*`/`plan`/`seatLimit`, `AuditLog`, `DeadLetterEvent`, `PushSubscription`, `CallLog`.
...
1. **Isolation-by-title-prefix is not isolation.** Manager-Cola / team-chat conversations once lived in the seller `Conversation`/`Message` tables distinguished only by a `[MANAGER_COLA]`/`[COMPANY_CHAT]` title prefix. Because a manager_owner also owns their personal seller space, *space ownership alone is not a boundary*. Any seller read of `Conversation`/`Message` MUST route through `lib/chat/conversation-access` (`isSellerConversation` / reserved-prefix exclusion) — list, single-conversation, messages, **and the write path** (`resolveConversation`). Manager chat now has its own `ManagerConversation`/`ManagerMessage` tables (keyed by `companyId`); prefer structural separation over string matches.
2. **`Space.companyId` is NOT "which company the user belongs to."** Membership lives in `CompanyMembership` (a user can be in several). `Space.companyId` is only the intake-config owner and must never be overwritten on a second join. Don't read it as a membership signal.
3. **Parallel seller↔manager routes drift.** `task`/`manager-task`, `conversations`/`manager-conversations`, `messages`/`manager-messages` are near-duplicates; a fix applied to one is easily missed on the other (the isolation guard gap lived in exactly this seam). When you touch one, check its twin.
...
5. **Service-role bypasses RLS.** All server queries use the service-role key, so RLS is *not* the tenant boundary — app-layer `.eq('spaceId'/'companyId')` scoping is. Add the scope filter; don't rely on a policy.
