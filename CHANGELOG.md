...
Chronological record of features, fixes, and changes in Cola. Most recent first.
...
- **Docs: Company feature spec** — Landed `docs/COMPANY_SPEC.md` in four chunks covering concepts + permissions + data model, feature map for BP1–BP7 and linear steps 1–3, and the full route inventory + audit-logging notes (`b680f91`, `02e2b5b`, `14cedb5`, `491a103`).
...
### Company tier (BP1–BP7)

Manager-side features delivered in seven phases. See `docs/COMPANY_SPEC.md` for the full feature map.

- **BP1 — agent offboarding** — Atomic RPC transfers contacts/deals/demos when removing a seller; dialog wired + vitest coverage (`6db1e83`, `345e46d`).
...
- **BP4 — deal-at-risk dashboard** — Risk view on the manager pipeline with colourblind-safe dots and a healthy-state strip (`b721de9`, `3417eb1`).
- **BP5 — review requests** — Schema + flag UI + first two APIs, then review detail APIs + manager queue UI, API coverage, clickable "Review pending" chip, and offboarding gate + notify enrichment (`a72e108`, `c1507c0`, `1463ed4`, `cd6c99b`, `6d05915`).
- **BP6 — template versioning** — Company templates schema + magic-Note extraction, GET/POST rewrite, `[id]` routes + tests, UI rewrite onto the new API, and security/correctness fixes (`46612ab`, `3246091`, `3e9196b`, `2571397`, `e274236`).
- **BP7 — lead routing** — Auto-routing schema on `Company`, routing engine + settings wiring + tests, plus a no-mutate fix on Supabase responses (`d3176d4`, `511adb5`, `380fd83`).

### Linear steps 1–3

- **Step 1 — Seller reviews** — New seller-side "My reviews" page surfacing the manager's review requests (`b524072`).
- **Step 2 — Audit-log viewer** — Manager audit-log viewer that preserves the team-activity feed (`733f4a5`).
...
- **Feature: Resend email notifications for demos** — All demo emails (confirmation, reminder, follow-up, agent notification) migrated from SMTP/Nodemailer to Resend, matching the design language of lead notification emails (dark header, detail boxes, CTA buttons).
- **Feature: Telnyx SMS notifications** — New SMS integration (`lib/sms.ts`) for real-time text notifications:
  ...
  - Demo bookings → guest confirmation SMS + agent alert SMS
...
- **Feature: Unified notification dispatcher** (`lib/notify.ts`) — Single entry point for all notifications. Checks both channel preferences (email on/off, SMS on/off) and per-event preferences before dispatching. Functions: `notifyNewLead`, `notifyNewDemo`, `notifyNewDeal`, `notifyNewContact`.
- **Feature: Per-event notification toggles** — Users can independently enable/disable notifications for each event type: new leads, demo bookings, new deals, follow-up reminders. These work as a second layer alongside the channel toggles.
...
- **Enhancement: Dashboard redesign** — Aligned manager dashboard with seller dashboard design patterns:
  ...
  - Max-width 1120px constraint matching seller dashboard
- **Feature: Manager onboarding checklist** — 4-step guided setup (invite sellers, configure settings, review performance, export report) with progress bar and localStorage-persisted dismissal.
- **Feature: Team activity feed** — New `TeamActivityFeed` component + `/api/manager/activity` API route showing recent leads, deals, and demos across all team members in a colored-dot timeline.
- **Feature: Search on sellers page** — Client-side search input filtering sellers by name or email.
...
- **Feature: Global search for manager users** — `GlobalSearch` and `NotificationCenter` now visible for all users with a workspace slug, including manager users.
...
- `notifyDemoBookings` boolean (default true)
...
- `components/manager/onboarding-checklist.tsx` — Manager onboarding checklist
- `components/manager/team-activity-feed.tsx` — Team activity timeline
- `components/manager/members-search.tsx` — Members page search wrapper
- `app/api/manager/activity/route.ts` — Manager team activity API
...
- Rewrote `README.md` from multi-tenant template to Cola-specific documentation covering features, tech stack, project structure, and setup
