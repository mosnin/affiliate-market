"""Contract + permission-gate tests for the 15 manager tools.

Phase 5 of Cola-for-Managers — defends three invariants:

  1. Every tool calls `require_manager_role(ctx)` first.
  2. Cross-company scope is enforced where applicable.
...
disabled so guard refusals re-raise as `ManagerPermissionError` instead of
being silently converted into model-visible strings.

Run from `agent/` with:

    NEXT_PUBLIC_SUPABASE_URL=stub SUPABASE_SERVICE_ROLE_KEY=stub \\
      python -m pytest tests/test_manager_tools.py -v
"""
...
# Make `import db`, `import security.context`, `import tools.manager.*` work
...
from tools.manager import MANAGER_TOOLS  # noqa: E402
from tools.manager._guards import ManagerPermissionError  # noqa: E402
from tools.manager import actions as actions_mod  # noqa: E402
from tools.manager import performance as performance_mod  # noqa: E402
from tools.manager import pipeline as pipeline_mod  # noqa: E402
from tools.manager import revenue as revenue_mod  # noqa: E402
from tools.manager import team as team_mod  # noqa: E402
...
def fake_db(monkeypatch: pytest.MonkeyPatch) -> _FakeSupabase:
    """Replace every `supabase()` import with a fake. Each manager tool module
...
def _ctx(
    *,
    manager_role: str = "manager_owner",
    company_id: str = "bk_a",
    user_id: str = "u_manager",
) -> AgentContext:
    return AgentContext(
        space_id="s_manager",
        space_name="Manager Workspace",
...
def _tool_by_name(name: str) -> Any:
    for t in MANAGER_TOOLS:
        if t.name == name:
            return t
    raise LookupError(f"No manager tool named {name!r}")
...
_ALL_TOOL_INPUTS: list[tuple[str, dict[str, Any]]] = [
    ("team_health", {}),
    ("seller_performance", {"seller_id": "u_seller"}),
    ("read_seller_morning_story", {"seller_id": "u_seller"}),
    ("find_stuck_deals", {}),
    ...
    ("find_at_risk_agents", {}),
    ("reassign_lead", {"lead_id": "c_lead", "to_seller_id": "u_seller"}),
    ("flag_deal_for_manager_review", {"deal_id": "d1", "reason": "needs a look here please"}),
    ("send_team_announcement", {"message": "stand-up at 9am"}),
    ("change_member_role", {"member_id": "m1", "new_role": "manager_admin"}),
...
@pytest.mark.asyncio
async def test_non_manager_ctx_is_refused(tool_name: str, args: dict[str, Any]) -> None:
    """Every manager tool refuses a non-manager caller before doing any work.
    ...
    ctx = _ctx(manager_role="seller_member")
    with pytest.raises(ManagerPermissionError):
...
async def test_empty_role_is_refused(tool_name: str, args: dict[str, Any]) -> None:
    """An empty manager_role (autonomous run / missing context) is refused."""
    tool = _tool_by_name(tool_name)
    ctx = _ctx(manager_role="")
    with pytest.raises(ManagerPermissionError):
        await _invoke(tool, ctx, args)


# ── 2. Cross-company scope ────────────────────────────────────────────────


@pytest.mark.asyncio
async def test_seller_performance_refuses_cross_company(fake_db: _FakeSupabase) -> None:
    """`seller_performance` runs against a seller whose Space lives in a
    DIFFERENT company — must refuse with ManagerPermissionError."""
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(
            data={"id": "s_other", "ownerId": "u_seller", "companyId": "bk_OTHER", "name": "Other Company"}
        ),
    )
    tool = _tool_by_name("seller_performance")
    ctx = _ctx(company_id="bk_a")
    with pytest.raises(ManagerPermissionError):
        await _invoke(tool, ctx, {"seller_id": "u_seller"})


@pytest.mark.asyncio
async def test_read_seller_morning_story_refuses_cross_company(fake_db: _FakeSupabase) -> None:
    """Same cross-company refusal on `read_seller_morning_story`."""
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_other", "ownerId": "u_seller", "companyId": "bk_OTHER"}),
    )
    tool = _tool_by_name("read_seller_morning_story")
    ctx = _ctx(company_id="bk_a")
    with pytest.raises(ManagerPermissionError):
        await _invoke(tool, ctx, {"seller_id": "u_seller"})
...
@pytest.mark.asyncio
async def test_reassign_lead_refuses_cross_company_contact(fake_db: _FakeSupabase) -> None:
    """`reassign_lead` returns ok=false when the contact is missing for this
    company. The TS path scopes contact lookup to company_id, and falls
    back to the manager owner's space — both lookups should miss for a
    cross-company lead. We mock contact lookup as None and ensure the
    Company row in `_resolve_manager_space` also returns no useful space.
    """
    # First contact lookup (by companyId): miss.
    fake_db.set_for("Contact", "select", _FakeResult(data=None))
    # `_resolve_manager_space` then queries Company; return a row but with
    # no matching space the second query won't help.
    fake_db.set_for(
        "Company",
        "select",
        _FakeResult(data={"id": "bk_a", "ownerId": "u_manager", "name": "B"}),
    )
    # The manager-owner's Space lookup yields a space (s_manager).
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_manager", "ownerId": "u_manager", "name": "Manager Space", "companyId": "bk_a"}),
    )
    # Second contact lookup (by manager_space.space_id): also miss.
    fake_db.set_for("Contact", "select", _FakeResult(data=None), )

    tool = _tool_by_name("reassign_lead")
    ctx = _ctx(company_id="bk_a")
    res = await _invoke(tool, ctx, {"lead_id": "c_other", "to_seller_id": "u_seller"})
    assert res["ok"] is False
    assert "company" in res["summary"].lower() or "not found" in res["summary"].lower()
...
async def test_change_member_role_first_call_requires_confirmation(fake_db: _FakeSupabase) -> None:
    ...
    fake_db.set_for(
        ...
        "select",
        _FakeResult(
            data={"id": "m1", "userId": "u_seller", "role": "seller_member", "companyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_seller", "name": "Alice", "email": "alice@x.com"}),
    )

    tool = _tool_by_name("change_member_role")
    ctx = _ctx(manager_role="manager_owner")
    res = await _invoke(tool, ctx, {"member_id": "m1", "new_role": "manager_admin"})
    assert res["ok"] is False
...
async def test_change_member_role_confirmed_executes(fake_db: _FakeSupabase) -> None:
    ...
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m1", "new_role": "manager_admin", "confirmed": True},
    )
    assert res["ok"] is True
    assert res["newRole"] == "manager_admin"
    assert any(c["table"] == "CompanyMembership" and c["op"] == "update" for c in fake_db.log)
...
async def test_offboard_member_first_call_requires_confirmation(fake_db: _FakeSupabase) -> None:
    ...
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_seller", "name": "Alice", "email": "alice@x.com", "status": "active"}),
    )
...
async def test_offboard_member_confirmed_calls_rpc(fake_db: _FakeSupabase) -> None:
    """Confirmed call fires the offboard_company_member RPC and audits."""
    # Target membership.
    fake_db.set_for(
        "CompanyMembership",
        "select",
        ...
        _FakeResult(
            data={"id": "m2", "userId": "u_dest", "role": "seller_member", "companyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    fake_db.set_for(
        "rpc:offboard_company_member",
        "rpc",
        _FakeResult(data=[{"contacts_moved": 4, "deals_moved": 2, "demos_moved": 1}]),
    )
    ...
    assert res["demosMoved"] == 1
    rpc_calls = [c for c in fake_db.log if c["op"] == "rpc"]
    assert len(rpc_calls) == 1
    assert rpc_calls[0]["payload"]["fn"] == "offboard_company_member"
    assert any(c["table"] == "AuditLog" and c["op"] == "insert" for c in fake_db.log)
...
async def test_reassign_lead_writes_audit(fake_db: _FakeSupabase) -> None:
    ...
    # Contact lookup by companyId succeeds.
    fake_db.set_for(
        "Contact",
        "select",
        _FakeResult(
    ...
    # `_resolve_manager_space` chain.
    fake_db.set_for(
        "Company",
        "select",
        _FakeResult(data={"id": "bk_a", "ownerId": "u_manager", "name": "B"}),
    )
    # Space lookups: manager's space, target seller's space.
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_manager", "ownerId": "u_manager", "name": "Manager", "companyId": "bk_a"}),
        _FakeResult(data={"id": "s_seller", "ownerId": "u_seller", "name": "Seller", "companyId": "bk_a"}),
    )
    # Membership for target seller.
    fake_db.set_for(
        "CompanyMembership",
        "select",
        _FakeResult(
            data={"id": "m_seller", "userId": "u_seller", "role": "seller_member"}
        ),
    )
    # Target user.
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_seller", "name": "Jordan", "email": "j@x.com"}),
    )
    ...
    res = await _invoke(
        tool,
        ctx,
        {"lead_id": "c1", "to_seller_id": "u_seller", "reason": "lighter load"},
    )
    assert res["ok"] is True
    audit_inserts = [
...
async def test_flag_deal_writes_audit(fake_db: _FakeSupabase) -> None:
    ...
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_seller", "ownerId": "u_seller", "companyId": "bk_a"}),
    )

    tool = _tool_by_name("flag_deal_for_manager_review")
    ctx = _ctx(manager_role="manager_owner")
    res = await _invoke(
        tool,
        ctx,
...
async def test_send_team_announcement_writes_audit(fake_db: _FakeSupabase) -> None:
    fake_db.set_for(
        "Company",
        "select",
        _FakeResult(data={"id": "bk_a", "ownerId": "u_manager", "name": "Apex"}),
    )
    fake_db.set_for(
        "Space",
        "select",
    ...
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_manager", "name": "Sam", "email": "sam@x.com"}),
    )
    fake_db.set_for(
        "CompanyMembership",
        "select",
        _FakeResult(
            data=[
                {"id": "m1", "userId": "u_manager", "role": "manager_owner"},
                {"id": "m2", "userId": "u_seller", "role": "seller_member"},
            ]
        ),
    )
...
async def test_set_routing_rule_writes_audit(fake_db: _FakeSupabase) -> None:
    ...
    audits = [c for c in fake_db.log if c["table"] == "AuditLog" and c["op"] == "insert"]
    assert len(audits) == 1
    assert audits[0]["payload"]["resource"] == "Company"
    ...
    updates = [c for c in fake_db.log if c["table"] == "Company" and c["op"] == "update"]
...
# ── 6. manager_admin can only move seller_member ⇄ seller_member ───────────
...
async def test_change_member_role_admin_cannot_change_other_admins(
    ...
    """A manager_admin caller cannot demote another manager_admin to
    seller_member — that move is owner-only."""
    fake_db.set_for(
        "CompanyMembership",
        "select",
        _FakeResult(
            data={"id": "m_other_admin", "userId": "u_admin2", "role": "manager_admin", "companyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    tool = _tool_by_name("change_member_role")
    ctx = _ctx(manager_role="manager_admin")
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m_other_admin", "new_role": "seller_member", "confirmed": True},
    )
    assert res["ok"] is False
    assert "owner" in res["summary"].lower()
    # No UPDATE happened — admin attempted owner-only move.
    assert not any(c["table"] == "CompanyMembership" and c["op"] == "update" for c in fake_db.log)
...
async def test_change_member_role_admin_cannot_promote_to_admin(
    ...
    """A manager_admin cannot promote a seller_member to manager_admin
    either — promotion-to-admin is owner-only."""
    fake_db.set_for(
        "CompanyMembership",
        "select",
        _FakeResult(
            data={"id": "m_seller", "userId": "u_seller", "role": "seller_member", "companyId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m_seller", "new_role": "manager_admin", "confirmed": True},
    )
    assert res["ok"] is False
    assert "owner" in res["summary"].lower()
...
async def test_offboard_member_admin_refused(fake_db: _FakeSupabase) -> None:
    """offboard_member is manager_owner-only — manager_admin gets refused."""
    tool = _tool_by_name("offboard_member")
    ctx = _ctx(manager_role="manager_admin")
    res = await _invoke(tool, ctx, {"member_id": "m1"})
    assert res["ok"] is False
...
async def test_set_routing_rule_rejects_invalid_strategy(fake_db: _FakeSupabase) -> None:
    ...
    # No UPDATE attempted.
    assert not any(c["table"] == "Company" and c["op"] == "update" for c in fake_db.log)
...
# ── Sanity: MANAGER_TOOLS has all 15 tools registered ────────────────────────


def test_manager_tools_registry_has_fifteen() -> None:
    assert len(MANAGER_TOOLS) == 15
    names = {t.name for t in MANAGER_TOOLS}
    expected = {
        "team_health",
        "seller_performance",
        "read_seller_morning_story",
        "find_stuck_deals",
        "find_unassigned_leads",
        "find_breached_leads",
        "commission_report",
