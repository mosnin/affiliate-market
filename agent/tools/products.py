"""Product tools — add a product, share a packet with a contact.
...
async def add_product(
    ...
    try:
        result = await db.table("Product").insert(row).execute()
...
async def send_product_packet(
    ...
    if not packet_id and not product_id:
        return {"error": "Pass packet_id or product_id"}
    ...
    if packet_id:
        pkt = await (
            db.table("ProductPacket")
            .select("id,name,token,productId,expiresAt,revokedAt")
            .eq("id", packet_id)
            .eq("spaceId", space_id)
            .maybe_single()
            .execute()
        ...
        if not pkt.data:
            return {"error": "ProductPacket not found in space"}
        packet = pkt.data
    else:
        prop = await (
            db.table("Product")
        ...
        if not prop.data:
            return {"error": "Product not found in space"}
        pkts = await (
            db.table("ProductPacket")
            .select("id,name,token,productId,expiresAt,revokedAt")
            .eq("spaceId", space_id)
        ...
        if not pkts.data:
            return {"error": (
                f"No packet exists for that product. Create a packet first "
