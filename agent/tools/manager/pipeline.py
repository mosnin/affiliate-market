"""Manager read tools — pipeline pressure points.

Two tools live here because they both answer "where's the team losing
momentum?":

  find_stuck_deals(min_days)         — active deals across the company that
                                       haven't been touched in `min_days`+
                                       days, ranked by age × value so the
                                       biggest stuck deals surface first.
  find_unassigned_leads(min_hours)   — company-intake leads sitting in the
                                       queue without a seller on them, with
                                       a proposed routing target picked by
                                       lightest current load.
  find_breached_leads()              — routed leads (tag: assigned-by-manager)
                                       that have never been contacted
                                       (lastContactedAt IS NULL) and are past
                                       the company's slaFirstResponseMinutes
                                       clock; flags any already past
                                       slaEscalateMinutes too.

Both are company-scoped through `Space WHERE companyId = ctx.company_id`
and gated by `require_manager_role(ctx)`. Phase 3 will add the matching write
tools (assign_lead, ping_seller_on_stuck_deal); for now these are read-only
surface — the manager still does the actual move themselves.
"""
...
# Cap on returned rows — the manager chat needs the top N, not the full list.
_STUCK_DEAL_LIMIT = 20
_UNASSIGNED_LEAD_LIMIT = 50
_BREACHED_LEAD_LIMIT = 50

# Default SLA window when the company row has no value set.
...
async def find_stuck_deals(
    ...
    """Find active deals across the company stalled min_days+, ranked by days*value."""
    ...
    # Resolve the company's spaces in one query — we filter Deal on
    ...
    # Resolve seller names for the assigned_to field.
    ...
    for d in deals:
        ...
        scored.append({
            ...
            "assigned_to": {
                "id": owner_id or "",
                "name": (owner or {}).get("name") or (owner or {}).get("email") or "Seller",
...
async def find_unassigned_leads(
    ...
    """Find unassigned company-intake leads, with the lightest-load seller proposed."""
    # Unassigned = Contact in this company tagged 'company-lead', not 'assigned', older than min_hours.
    ...
    # Unassigned leads — same predicates as app/manager/leads/page.tsx.
    leads_res = await (
        db.table("Contact")
    ...
    unassigned = [
        ...
        if "company-lead" in (c.get("tags") or [])
    ...
    # Compute open_load per seller — duplicate of team_health's logic so
    ...
    seller_loads: list[dict[str, Any]] = []
    for space in spaces:
        space_id = space["id"]
    ...
    # Sort ascending by open_load; the lightest seller is at the top.
    seller_loads.sort(key=lambda r: r["open_load"])
    lightest = seller_loads[0]

    # Team median for the "vs benchmark" framing in the reason string.
    loads = [r["open_load"] for r in seller_loads]
...
async def find_breached_leads(
    ...
    """Find routed leads past the company's first-response SLA, oldest first."""
    # Breached = Contact in a member Space tagged 'assigned-by-manager',
    ...
    sla_first_minutes: int = int(
        company.get("slaFirstResponseMinutes") or _DEFAULT_SLA_FIRST_RESPONSE_MINUTES
    )
    sla_escalate_minutes: int | None = (
        int(company["slaEscalateMinutes"])
        if company.get("slaEscalateMinutes") is not None
        else None
    )
    ...
    if not spaces:
        return {
            "ok": True,
            "breached_leads": [],
            "summary": "No member spaces found for this company.",
        }
    ...
    #   - carry the 'assigned-by-manager' tag  (filtered in Python — same
    ...
    breached = [
        c for c in raw
        if "assigned-by-manager" in (c.get("tags") or [])
    ...
    for c in breached:
        ...
        owner = users_by_id.get(owner_id) if owner_id else None
        seller_name = (
            (owner or {}).get("name") or (owner or {}).get("email") or "Unknown Seller"
        )
