"""Manager read tools — commission revenue across the company.

One tool: commission_report(period). Sums GCI by seller for a chosen window
and returns the company total. GCI = value * commissionRate / 100, same
...
to won, the same proxy `/api/manager/morning` uses for won-this-week
...
async def commission_report(
    ...
    """Commission revenue across the company, broken out by seller + total."""
    ...
    # Aggregate per seller (one row per space owner, even if the seller
    # has zero wins in the window — calm chief-of-staff reporting needs
    # the full team list so the manager can see who has nothing on the board).
    rollup: dict[str, dict[str, Any]] = {}
    for owner_id in owner_ids:
        u = users_by_id.get(owner_id) or {}
        rollup[owner_id] = {
            "id": owner_id,
            "name": u.get("name") or u.get("email") or "Seller",
            "deals_closed": 0,
            "gci_earned": 0.0,
        }
    ...
    by_seller.sort(key=lambda r: r["gci_earned"], reverse=True)
