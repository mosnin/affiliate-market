"""Manager read tools — team-wide capacity, per-seller performance, morning mirror.

Three tools live here because they all answer the manager's question "what's
happening on the team?" from three zoom levels:

  team_health()                 — every seller at a glance, with a load_signal
                                  classification per person and the team median
                                  response speed as a context benchmark.
  seller_performance(id)       — drill into one seller: deals by stage, GCI,
                                  conversion percentage, response speed, last
                                  activity. Cross-company drill is refused.
  read_seller_morning_story(id) — mirror what the seller sees on their own
                                  /cola home, scoped to that seller's space.
                                  PII-safe: the manager sees the seller's
                                  surface, not their private notes/threads.

All three call `require_manager_role(ctx)` first (defense layer 3) and scope
every read through `Space WHERE companyId = ctx.company_id`. Cross-
company queries return `ManagerPermissionError`, not empty data — a silent
empty is indistinguishable from a permissions bug.

The team-median + load_signal heuristics are intentionally simple:
  - one number per seller (open_load = active_deals + open_leads)
  - one number per seller (avg_response_hours_7d)
  - team median is the seven-day company median, used as a "vs benchmark"
    framing in the manager's chat so Cola reports facts, not judgments.
"""
...
from ._guards import ManagerPermissionError, require_manager_role


# ── Shared constants ────────────────────────────────────────────────────────

# Activity types that count as outbound contact with a human. Matches the
# convention in `app/api/manager/morning/route.ts`:
...
async def _team_spaces(company_id: str) -> list[dict[str, Any]]:
    """Resolve the company's seller spaces in one query.

    Returns rows shaped {id, ownerId, name}. Empty list if the company has
    no spaces yet. The caller pairs ownerId → User to get seller names; we
...
async def _response_hours_for_space(
    ...
    The seller side never sees raw values — they roll up into a median.
    """
    db = await supabase()
    ...
    # how fast did the seller get back to the lead after it arrived?
    contacts_res = await (
        db.table("Contact")
        .select("id,createdAt")
        .eq("spaceId", space_id)
...
def _load_signal(open_load: int, avg_response_hours: float | None, last_active: datetime | None, now: datetime) -> str:
    """Classify a seller's load. See docstring on team_health for thresholds."""
...
async def team_health(ctx: RunContextWrapper[AgentContext]) -> dict[str, Any]:
    """Read every seller's current capacity, load, and 7-day response speed."""
    # Per-seller: active_deals, open_leads, avg_response_hours_7d, load_signal.
    ...
    spaces = await _team_spaces(company_id)
    if not spaces:
        return {
            "ok": True,
            "sellers": [],
    ...
    # Per-seller stats as a coroutine so the roster runs in PARALLEL —
    # the sequential version paid 5 DB round-trips per seller in series
    ...
    async def _stats(space: dict[str, Any]) -> tuple[dict[str, Any], list[float]]:
        ...
        name = (user or {}).get("name") or (user or {}).get("email") or "Seller"
        ...
        # Response hours for this seller.
    ...
    sellers: list[dict[str, Any]] = [entry for entry, _ in gathered]
...
async def seller_performance(
    ...
    """Deep-dive on one seller: deals by stage/status, conversion, YTD GCI, response speed."""
    # Cross-company drill raises ManagerPermissionError.
    ...
    # Cross-company guard — resolve the seller's space and confirm its
    # companyId matches the caller's. Empty data = treat as forbidden:
    # we don't want a leak signal that "this user exists but isn't in your
    # company."
    space_res = await (
        db.table("Space")
        .select("id,ownerId,companyId,name")
        .eq("ownerId", seller_id)
        .maybe_single()
        .execute()
    )
    if not space_res.data or space_res.data.get("companyId") != company_id:
        raise ManagerPermissionError(
            "That seller isn't in your company."
        )
    ...
    if not user_res.data:
        ...
        return {
            "ok": True,
            "seller": {"id": seller_id, "name": "Seller", "joinedAt": None},
            "deals_by_stage": {},
            "deals_by_status": {},
            "conversion_pct": None,
            "gci_ytd": 0.0,
    ...
    # All deals for this seller — small enough to roll up in Python.
    deals_res = await (
        db.table("Deal")
    ...
    return {
        ...
        "seller": {
            "id": user["id"],
            "name": user.get("name") or user.get("email") or "Seller",
...
@function_tool(strict_mode=False)
async def read_seller_morning_story(
    ctx: RunContextWrapper[AgentContext],
    seller_id: str,
) -> dict[str, Any]:
    """Mirror the seller's /cola morning briefing; counts + named subjects only."""
    ...
    # Cross-company guard — same pattern as seller_performance.
    space_res = await (
        db.table("Space")
    ...
    user = user_res.data or {"id": seller_id, "name": "Seller"}
    seller_name = user.get("name") or user.get("email") or "Seller"

    now = datetime.now(timezone.utc)
    week_out = now + timedelta(days=7)

    # ── Counts (mirroring /api/agent/morning shape) ─────────────────────
    # The seller-side route excludes company-routed leads. We mirror
    # that filter so the manager sees what's actually on the seller's
    # personal desk, not aggregate company flow.
    ...
    # updatedAt > 30 days ago counts as stuck. The seller route uses the
...
# Module-level list — `__init__.py` extends MANAGER_TOOLS from this.
TEAM_TOOLS: list = [team_health, seller_performance, read_seller_morning_story]
