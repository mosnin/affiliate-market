"""Manager read tools — response-time benchmarking across the team.

Two tools live here because they both answer "how is the team performing?":

  audit_response_times()      — per-seller median first-touch response time
                                over 30d, banded vs team median (fast /
                                on_pace / slow / no_data).
  find_at_risk_agents()       — flight-risk signals per seller: won-deal
...
staff voice (see `agent/cola_manager.py:MANAGER_INSTRUCTIONS`) frames every
seller's number against the team benchmark — never as a judgment. "Alice's
median is 28h vs team median 6h" is OK. "Alice is slow" is not.

Definition of "outbound" matches the rest of the manager side: ContactActivity
...
async def audit_response_times(
    ...
    """Per-seller median first-touch response time over 30d, banded vs team median."""
    # Bands relative to team_median: fast (<=0.5x), on_pace (mid), slow (>=2x).
    # Sellers with no outbound in window surface as no_data, not slow.
    ...
    # Per-seller audit as a coroutine — the roster runs in PARALLEL via
    # asyncio.gather instead of 2 sequential DB round-trips per seller.
    async def _audit(space: dict[str, Any]) -> tuple[dict[str, Any], list[float]] | None:
        ...
        # a shared module would be nice, but the seller tools follow the
    ...
    per_seller: list[dict[str, Any]] = [r[0] for r in audited if r is not None]
    ...
    # the manager wants the team's "typical" response, which is best measured
    # on raw samples).
    ...
    if team_median is None:
        # No outbound across the company — return everyone in no_data.
        for r in per_seller:
...
# Silence threshold: if a seller has had no activity (Contact or Deal
...
# Tag written by the manager assignment flow (same as find_breached_leads).
_ASSIGNED_BY_MANAGER_TAG = "assigned-by-manager"
...
async def find_at_risk_agents(
    ...
    """Flight-risk signals per seller: won-deal trend, silence days, un-worked leads, SLA hits."""
    # Signals per seller:
    ...
    #   3. unworked_assigned_leads — 'assigned-by-manager' contacts with lastContactedAt IS NULL.
    #   4. sla_nudge_count / sla_escalate_count — contacts carrying the SLA tags.
    # Sellers with ≥1 signal surface in the at_risk list, sorted by signal severity.
    require_manager_role(ctx)
    company_id = ctx.context.company_id
    ...
    # Per-seller assessment as a coroutine so the whole roster runs in
    # PARALLEL via asyncio.gather. The sequential version paid ~6 DB
    # round-trips per seller in series — 120 sequential queries for a
    # 20-agent company, the dominant term in "manager chat is slow".
    async def _assess(space: dict[str, Any]) -> dict[str, Any] | None:
        ...
        unworked_count = sum(
            ...
            if _ASSIGNED_BY_MANAGER_TAG in (c.get("tags") or [])
        )
        ...
        if not reasons:
            return None  # no signals — seller is fine
