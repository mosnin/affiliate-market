...
This agent serves both surfaces:
  - chat (the seller talks to Cola via /api/ai/task → Modal chat_turn)
  - autonomous (event triggers fire run_agent_for_space → same agent)

The opening message tells Cola which mode it's in.
...
Tool surface (36 tools):
  ...
  - book_demo
  - route_lead
  - add_product / send_product_packet
...
from tools.products import add_product, send_product_packet
from tools.questions import ask_seller
from tools.routing import route_lead
from tools.demos import book_demo
...
- CHAT — seller sent a message. Identify the job, run tools, answer.
...
what the seller has connected. When the seller names a connected
...
every connected toolkit. NEVER tell the seller you lack access to a
...
- After draft_message: quote or paraphrase the `nextStep` string. The
  seller must know it's drafted, awaiting approval, and whether a
...
seller cares about (ambiguous recipient, real subject for serious
outreach, body of a follow-up with no context). One sentence, one
question, no menu. Don't re-confirm what the seller explicitly asked.

# Boundaries
- Never reveal internal IDs, API keys, or raw row metadata. Use names.
- Never claim a write you didn't execute.
- Never change deal status, value, or title from chat — seller's call.
...
async def load_ai_profile(space_id: str, db) -> str | None:
    ...
    try:
        ...
        if profile.get("displayName"):
            parts.append(f"The seller's preferred name: {profile['displayName']}")
...
def make_cola_agent(
    ...
    """
    Build the single Cola agent. Constructed fresh per run.

    `extra_tools` lets the caller append integration tools loaded per
    seller (Gmail, Slack, HubSpot, etc. via Composio). Native CRM tools
    ...
    workspace name) so the model can drop the seller's intake link into
    ...
    universally-stable text first and per-space content after:
      1. COLA_INSTRUCTIONS — identical across every space + every run.
         Caches once per OpenAI organization at runtime; every seller's
      ...
      3. ai_profile_text — per-space, slowly changing (seller edits their
         profile occasionally). Caches per-space until they edit it.
    """
    configure_agents_sdk()
    parts: list[str] = [COLA_INSTRUCTIONS]
    if workspace_info:
    ...
    base_tools = [
        ...
        # Goals
        manage_goal,
        # Routines — standing instructions Cola runs on a schedule
        manage_routines,
        # Drafts + outcomes
        draft_message,
        send_email_now,
