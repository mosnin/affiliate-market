/**
 * Route-level test for POST /api/cola/post-demo.
 *
...
const {
  ...
  loadPostDemoIntegrationToolsMock,
} = vi.hoisted(() => ({
  proposeActionsMock: vi.fn(),
  attachHumanSummariesMock: vi.fn(),
  loadPostDemoIntegrationToolsMock: vi.fn(),
}));
vi.mock('@/lib/cola/post-demo', async () => {
  const actual =
    await vi.importActual<typeof import('@/lib/cola/post-demo')>('@/lib/cola/post-demo');
  return {
    ...actual,
    proposeActions: proposeActionsMock,
    attachHumanSummaries: attachHumanSummariesMock,
    loadPostDemoIntegrationTools: loadPostDemoIntegrationToolsMock,
  };
});
...
import { POST } from '@/app/api/cola/post-demo/route';
...
function makeReq(body: unknown): NextRequest {
  return new NextRequest('http://localhost/api/cola/post-demo', {
    method: 'POST',
...
describe('POST /api/cola/post-demo', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.OPENAI_API_KEY = 'sk-test';
    // Default to no connected apps — the graceful-degradation baseline.
    loadPostDemoIntegrationToolsMock.mockResolvedValue([]);
  });
  ...
  it('returns proposals on happy path', async () => {
    ...
    proposeActionsMock.mockResolvedValue([
      { tool: 'log_call', args: { personId: 'p1', summary: 'Demo' }, summary: 'Log call', mutates: true },
  ...
  it('forwards a contextHint with personId/dealId to the orchestrator', async () => {
    // Surfaces the URL-driven pre-fill — when the seller opens the recorder
  ...
  it('forwards integration tools to the orchestrator when the seller has connected apps', async () => {
    // The Gmail-connected case: the orchestrator must be told the
    // GMAIL_SEND_EMAIL tool exists, otherwise the post-demo stack can
    ...
    proposeActionsMock.mockResolvedValue([
      {
        ...
        args: { to: 'sam@chen.com', subject: 'Demo follow-up', body: '...' },
    ...
    expect(loadPostDemoIntegrationToolsMock).toHaveBeenCalledOnce();
    expect(loadPostDemoIntegrationToolsMock).toHaveBeenCalledWith({
      spaceId: 'space_1',
      userId: 'user_1',
    });
