 ...
 * Python sandbox (opt-in via `COLA_CHAT_RUNTIME=modal`).
 *
...
const ORIGINAL = process.env.COLA_CHAT_RUNTIME;

describe('chatRuntime', () => {
  afterEach(() => {
    if (ORIGINAL === undefined) delete process.env.COLA_CHAT_RUNTIME;
    else process.env.COLA_CHAT_RUNTIME = ORIGINAL;
  });
  ...
  it('returns "ts" for any value other than the exact string "modal"', () => {
    ...
    process.env.COLA_CHAT_RUNTIME = ' modal ';
    expect(chatRuntime()).toBe('ts');
    process.env.COLA_CHAT_RUNTIME = 'true';
    expect(chatRuntime()).toBe('ts');
    process.env.COLA_CHAT_RUNTIME = '1';
    expect(chatRuntime()).toBe('ts');
