...
describe('shapeProposals', () => {
  ...
  it('drops tools outside the post-demo allowlist', () => {
    const out = shapeProposals({
      proposals: [
        { tool: 'find_product', args: {} }, // not on allowlist
        { tool: 'pipeline_summary', args: {} }, // not on allowlist
        { tool: 'log_call', args: { personId: 'abc12345', summary: 'Demo' } },
      ],
    });
...
describe('shapeProposals — integration tools', () => {
  ...
  it('keeps integration slugs that match the seller\'s connected apps', () => {
    const out = shapeProposals(
      {
        proposals: [
          { tool: 'GMAIL_SEND_EMAIL', args: { to: 'sam@chen.com', subject: 'Demo follow-up', body: 'Loved meeting you' } },
        ],
      },
  ...
  it('drops integration slugs the seller has NOT connected', () => {
  ...
  it('keeps native and integration tools side by side', () => {
    const out = shapeProposals(
      {
        proposals: [
          { tool: 'log_call', args: { personId: 'p1', summary: 'demo' } },
          { tool: 'GMAIL_SEND_EMAIL', args: { to: 'sam@chen.com' } },
          { tool: 'GOOGLECALENDAR_CREATE_EVENT', args: { summary: 'follow-up demo' } },
        ],
      },
  ...
  it('produces a clean seller-voice summary without "via Gmail"', () => {
    const out = shapeProposals(
      { proposals: [{ tool: 'GMAIL_SEND_EMAIL', args: { to: 'sam@chen.com' } }] },
      { integrationTools: [GMAIL] },
    );
    expect(out[0].summary).not.toMatch(/via gmail/i);
    // Verb is the truth: "Email" tells the seller what's happening.
...
describe('sellerVerbForToolkit / doneVerbForToolkit', () => {
  it('returns concise verbs for known toolkits', () => {
    expect(sellerVerbForToolkit('gmail')).toBe('Email');
    expect(sellerVerbForToolkit('googlecalendar')).toBe('Add to calendar');
  ...
  it('falls back gracefully for unknown toolkits', () => {
    expect(sellerVerbForToolkit('weird-app')).toBe('Run connected action');
