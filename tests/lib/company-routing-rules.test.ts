 ...
 * company lead-routing engine.
 *
 * The engine under test is `routeCompanyLead(companyId, lead?)` in
 * lib/company-routing.ts. With the BP7d layer added, enabled
 ...
 *
 * Mock shape mirrors tests/lib/company-routing.test.ts — a
 * table-keyed chain mock on `@/lib/supabase` with `mockByTable[name] =
 * { rows, single, error }` overrides.
 *
 * NB on pool-tag narrowing: CompanyMembership has no `tags` column
...
/**
 * Seed a canonical "three eligible sellers" fixture: u_a, u_b, u_c each
 * with a Space inside company b_1. Additional overrides can swap in
...
async function callRouter(lead?: {
  ...
  return mod.routeCompanyLead('b_1', lead);
}
...
// Criteria matching
// ──────────────────────────────────────────────────────────────────────────
describe('routeCompanyLead + rules — criteria', () => {
...
// Priority & enabled
// ──────────────────────────────────────────────────────────────────────────
describe('routeCompanyLead + rules — ordering & flags', () => {
...
// ──────────────────────────────────────────────────────────────────────────
describe('routeCompanyLead + rules — destination fallback', () => {
...
// ──────────────────────────────────────────────────────────────────────────
describe('routeCompanyLead + rules — pool method', () => {
  it('pool round-robin over the full seller set (no tag narrowing yet)', async () => {
  ...
  it.skip('pool method + destinationPoolTag narrows pool to tagged agents (TODO: requires CompanyMembership.tags column)', async () => {
    // The schema accepts destinationPoolTag but the v1 engine ignores it
    // because there is no tags column on CompanyMembership or User yet.
