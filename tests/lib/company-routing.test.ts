/**
 * BP7d — vitest coverage for the company lead-routing engine (BP7b).
 *
 * The engine under test is `routeCompanyLead(companyId)` in
 * lib/company-routing.ts. Three sibling agents (BP7a schema, BP7b
 * engine + integration, BP7c settings) are wiring this feature end-to-
 * end in parallel; this file owns unit coverage only.
 *
 * Mock shape mirrors tests/lib/manager-templates.test.ts:
 ...
 *
 * One deviation from manager-templates.test.ts: the engine calls
 * `.maybeSingle()` on the Company row for config load. Our `singleThen`
...
// `updateCalls.Company` in particular.
...
function makeCompany(overrides: Partial<CompanyConfigRow> = {}): CompanyConfigRow {
  return {
...
/**
 * Seed a canonical "three eligible sellers" fixture:
 *   agents u_a, u_b, u_c, each with a Space inside company b_1.
 * Tests that care about a different topology override `mockByTable`
 * entries directly.
 */
function seedThreeAgents(
  config: Partial<CompanyConfigRow> = {},
  opts: { extraUsers?: Array<Record<string, unknown>> } = {},
): void {
  mockByTable.Company = { single: makeCompany(config) };
  mockByTable.CompanyMembership = {
    rows: [
      { userId: 'u_a', role: 'seller_member' },
      { userId: 'u_b', role: 'seller_member' },
      { userId: 'u_c', role: 'seller_member' },
    ],
  };
  mockByTable.User = {
    rows: [
  ...
  mockByTable.Space = {
    rows: [
      { id: 's_a', ownerId: 'u_a', companyId: 'b_1' },
      { id: 's_b', ownerId: 'u_b', companyId: 'b_1' },
      { id: 's_c', ownerId: 'u_c', companyId: 'b_1' },
    ],
  };
}

async function callRouter(): Promise<unknown> {
  const mod = await import('@/lib/company-routing');
  return mod.routeCompanyLead('b_1');
}
...
// Gating
// ──────────────────────────────────────────────────────────────────────────
describe('routeCompanyLead — gating', () => {
  it('returns null when autoAssignEnabled is false', async () => {
    mockByTable.Company = {
      single: makeCompany({ autoAssignEnabled: false, assignmentMethod: 'round_robin' }),
    };
    const result = await callRouter();
    expect(result).toBeNull();
    // No side-effect write to Company.
    expect(updateCalls.Company ?? []).toHaveLength(0);
  });

  it("returns null when assignmentMethod is 'manual'", async () => {
    mockByTable.Company = {
      single: makeCompany({ autoAssignEnabled: true, assignmentMethod: 'manual' }),
    };
    const result = await callRouter();
    expect(result).toBeNull();
    expect(updateCalls.Company ?? []).toHaveLength(0);
  ...
  it('returns null when no seller_members exist', async () => {
    mockByTable.Company = {
      single: makeCompany({ autoAssignEnabled: true, assignmentMethod: 'round_robin' }),
    };
    mockByTable.CompanyMembership = { rows: [] };
  ...
  it('returns null when the only seller has no Space in this company', async () => {
    mockByTable.Company = {
      single: makeCompany({ autoAssignEnabled: true, assignmentMethod: 'round_robin' }),
    };
    mockByTable.CompanyMembership = {
      rows: [{ userId: 'u_orphan', role: 'seller_member' }],
    };
    mockByTable.User = { rows: [{ id: 'u_orphan', status: 'active' }] };
    // Space exists, but the engine filters by `companyId=this` — returning an
    // empty `rows` for Space is the simplest way to simulate a cross-tenant
    // miss because the chain mock doesn't actually filter by `eq('companyId')`.
    mockByTable.Space = { rows: [] };
    const result = await callRouter();
    expect(result).toBeNull();
    expect(updateCalls.Company ?? []).toHaveLength(0);
  ...
  it('fails closed on infra errors (Company select errors → null, no update)', async () => {
    mockByTable.Company = {
...
// Round-robin
// ──────────────────────────────────────────────────────────────────────────
describe('routeCompanyLead — round_robin', () => {
  ...
  it('updates lastAssignedUserId after picking (fire-and-forget)', async () => {
    ...
    await Promise.resolve();
    const writes = updateCalls.Company ?? [];
  ...
  it('ignores offboarded agents (eligibility filter)', async () => {
    ...
    mockByTable.Company = {
      single: makeCompany({ assignmentMethod: 'round_robin', lastAssignedUserId: null }),
    };
    mockByTable.CompanyMembership = {
      rows: [
        { userId: 'u_a', role: 'seller_member' },
...
// Score-based
// ──────────────────────────────────────────────────────────────────────────
describe('routeCompanyLead — score_based', () => {
  /**
   * Build a Contact rows array that yields `count` active leads per space.
   * Active = type IN QUALIFICATION/DEMO/APPLICATION, snoozedUntil IS NULL.
   */
