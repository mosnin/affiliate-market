...
--   SpaceSetting (per-agent custom forms), Company (company-level templates),
...
-- Where the form config came from: 'custom' (agent built it), 'company'
-- (inherited from company template), or 'legacy' (hardcoded form)
ALTER TABLE "SpaceSetting"
  ADD COLUMN IF NOT EXISTS "formConfigSource" text NOT NULL DEFAULT 'legacy'
    CHECK ("formConfigSource" IN ('custom', 'company', 'legacy'));

-- ============================================================
-- Company: company-level form template
-- ============================================================

-- Companies can define a template form that member agents inherit
ALTER TABLE "Company"
  ADD COLUMN IF NOT EXISTS "companyFormConfig" jsonb DEFAULT NULL;
...
-- GIN index on companyFormConfig for JSONB queries
