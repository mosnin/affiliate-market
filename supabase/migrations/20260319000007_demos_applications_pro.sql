-- Migration: Add tables for demo feedback, cancel tokens, and application status tracking
-- Run in Supabase SQL Editor

-- Demo feedback from guests after completed demos
CREATE TABLE IF NOT EXISTS "DemoFeedback" (
  id              text PRIMARY KEY DEFAULT gen_random_uuid()::text,
  "demoId"        text UNIQUE NOT NULL REFERENCES "Demo"(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_demo_feedback_demo ON "DemoFeedback" ("demoId");
CREATE INDEX IF NOT EXISTS idx_demo_feedback_space ON "DemoFeedback" ("spaceId");

ALTER TABLE "DemoFeedback" ENABLE ROW LEVEL SECURITY;

-- Demo manage tokens for guest self-service cancel/reschedule
ALTER TABLE "Demo" ADD COLUMN IF NOT EXISTS "manageToken" text UNIQUE;
