-- ============================================================================
-- Company Offboarding: atomic member transfer
-- ============================================================================
-- When a company offboards a member (seller leaves the firm, manager is
-- removed, etc.), the company-scoped records owned by that member — the
-- contacts the company considers its own, plus the deals / activities /
-- checklist items / demos that hang off those contacts — must be re-homed to
...
--   2. Defines `offboard_company_member(...)`, a SECURITY DEFINER function
--      that performs (or previews, via dry-run) the re-parenting of every
--      company-scoped row in a single transaction. If any statement fails,
...
-- is responsible for authorization (only manager_owner / manager_manager may
...
-----------------------------------------------------------------------
-- 2. offboard_company_member — atomic transfer + state update
-----------------------------------------------------------------------
...
BEGIN
  ...
  IF p_dry_run THEN
    ...
    SELECT COUNT(*) INTO v_open_demo_count
      FROM "Demo" t
    ...
    RETURN json_build_object(
      ...
      'openDemoCount', v_open_demo_count
    );
  END IF;
  ...
  -- (a) Move company-scoped contacts; capture their ids.
  ...
  -- (f) Move Demos that are in the leaving space AND for a moved contact.
  WITH moved AS (
    UPDATE "Demo"
       SET "spaceId" = v_destination_space_id
     WHERE "spaceId" = v_leaving_space_id
  ...
  -- (h) Remove the company membership row — access-block agent also keys
  --     off User.status, but dropping the membership makes the severance
  --     explicit and causes any company-scoped queries to fail closed.
