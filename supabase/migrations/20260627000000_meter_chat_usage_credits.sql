-- ============================================================================
-- Model-aware credit metering for every agent/chat turn (seller + company).
--
...
--
-- ONE trigger on ChatUsage covers EVERY path: seller direct (direct-stream),
-- seller agent in-process (sdk-chat-stream), manager direct (manager-direct), and
-- seller+manager via Modal (agent/ledger.py) — they all INSERT one ChatUsage row
...
-- the company pool only when the company is on a team plan AND the space
