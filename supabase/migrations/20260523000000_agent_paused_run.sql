...
-- checkpoint: the seller asked Cola to do something, a tool needs approval,
-- the run is suspended until they decide. Drafts are "Cola composed
...
-- when the seller decides. Schema-versioned by the SDK itself
...
--
-- The approvals JSONB is the seller-facing prompt list — `ApprovalPrompt[]`
...
CREATE TABLE IF NOT EXISTS "AgentPausedRun" (
  ...
  -- Clerk userId of the seller whose chat is paused. Not a Space-scoped
...
-- Per-user filter for sessions where multiple sellers share a Space.
