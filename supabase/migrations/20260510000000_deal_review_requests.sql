-- ============================================================================
-- Deal Review Requests: Manager Sign-Off On High-Stakes Deals
-- ============================================================================
-- WHY: Today managers can't be pinged for sign-off on a specific deal. Agents
-- run high-stakes transactions (unusual commission splits, aggressive price
-- concessions, atypical contingencies) past their manager over Slack or in
-- person, leaving no audit trail and no structured queue on the manager side.
-- High-stakes deals effectively fly blind — the manager only finds out when
-- something goes wrong at closing.
--
-- BP5 introduces a deal-scoped review flow: an agent clicks "Flag for manager
-- review" on a deal, a DealReviewRequest row lands here, a ManagerNotification
-- fires (handled by the API agent), and the manager works through the queue at
-- /manager/reviews. DealReviewComment captures the back-and-forth so the
...
-- can't spam the manager queue with duplicate flags on the same deal.
-- ============================================================================

-- 1. DealReviewRequest — one row per "flag this deal for manager review".
...
-- 2. DealReviewComment — threaded back-and-forth between manager and agent.
...
--    (a) Manager's queue: filter by company + status, usually status='open'.
...
--    (company membership, agent-owns-deal, manager-owns-company) at the
