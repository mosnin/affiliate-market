-- ═══════════════════════════════════════════════════════════════════════════
-- ProfilePage.featuredProductIds — seller-curated set + order for the
-- featured listings carousel on the public /p/[slug] page.
--
-- Before this column, /p/[slug] auto-pulled the 6 most-recently-updated
-- active listings. The seller had no say. Now they pick the exact set and
...
ALTER TABLE "ProfilePage"
  ADD COLUMN IF NOT EXISTS "featuredProductIds" text[] NOT NULL DEFAULT '{}';
