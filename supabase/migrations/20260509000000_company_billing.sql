-- ============================================================================
-- Company Billing: Per-Company Seat-Based Subscriptions
-- ============================================================================
-- WHY: Previously, billing was tied to the manager_owner's personal Space. This
...
--
-- Moving the subscription, plan, and seatLimit onto Company itself closes
-- all three gaps: seats are counted against the company row, usage is
-- directly queryable, and the manager_owner's personal Space billing becomes
-- independent. Note that Space.stripe* columns are intentionally retained —
-- solo sellers (non-manager accounts with no Company) still bill through
-- their Space, and the Stripe webhook will route incoming events by metadata
-- to either a Company or a Space depending on which flow created the sub.
-- ============================================================================

-- 1. Extend Company with billing + plan columns
ALTER TABLE "Company"
...
-- 3. Bootstrap existing companies' billing from their manager_owner's personal
--    Space subscription — but ONLY for rows that don't already have a
--    subscription on the Company side. One-shot, idempotent by the guard.
--    Space.stripe* columns are intentionally left intact; solo sellers (users
--    with no Company) still bill through their Space.
UPDATE "Company" b
...
--    no Stripe ids). Stripe-webhook agent: use these to resolve a Company
