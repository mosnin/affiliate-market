-- CMA reports — a seller generates a Comparative Market Analysis from a
-- subject product + comparable Product rows already in their workspace, then
-- shares a public link with a seller.
--
-- Fully in-house: comps are selected from the seller's own Product table (no
...
-- page renders a stable report even if the underlying Product rows later
-- change or get deleted. shareToken gates the public /cma/[token] route the way
-- ProductPacket.token gates /packet/[token].
--
-- RLS is enabled with no policies: every read/write goes through the server
-- with the service-role key (same posture as SupportTicket + ProductPacket),
...
-- The seller's "my CMAs" list reads newest-first within a space.
