-- ============================================================================
-- Company Deal Routing Rules: Schema (Phase BP7d — rules layer)
-- ============================================================================
-- The BP7a/b lead-routing engine picks a seller_member via a single global
-- choice on the Company row: manual, round_robin, or score_based. That
-- works for homogeneous teams, but companies with territorial or
...
--   1. Load DealRoutingRule rows WHERE companyId=? AND enabled=true
...
--      the agent is still eligible (active member of this company with
...
--      Company.assignmentMethod behaviour (round_robin / score_based /
...
CREATE TABLE IF NOT EXISTS "DealRoutingRule" (
  ...
  -- that agent is no longer eligible (offboarded, left the company, no
  ...
  -- the tag is NULL the pool is the full seller_member set.
  --
  -- NOTE: as of this migration, neither User nor CompanyMembership has
  ...
  -- seller pool"). A follow-up migration can add
  -- CompanyMembership.tags text[] and the engine will start honouring
