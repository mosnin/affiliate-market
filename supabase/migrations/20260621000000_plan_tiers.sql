...
--
-- Adds the Space-level plan tier (free|solo|pro) and extends the Company plan
...
--
-- ✓ VALIDATED on PostgreSQL 16 (applies clean against Space/Company; CHECK
...
-- ── Company: extend plan enum with team|team_plus ─────────────────────────
-- Drop whatever CHECK currently constrains Company.plan (name may vary), then
...
BEGIN
  ...
  IF c IS NOT NULL THEN
    EXECUTE format('ALTER TABLE "Company" DROP CONSTRAINT %I', c);
  END IF;
END $$;

ALTER TABLE "Company" ADD COLUMN IF NOT EXISTS "planActivatedAt" timestamptz;
ALTER TABLE "Company"
  ADD CONSTRAINT "Company_plan_check"
