...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) {
    ...
    logger.error('[products/PATCH] update failed', { productId: id }, error);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // when the row is removed — otherwise listing photos for sold products
  // survive forever in storage, EXIF and all.
  const rawPhotos = (ctx.product as { photos?: unknown }).photos;
  ...
  if (error) {
    logger.error('[products/DELETE] failed', { productId: id }, error);
