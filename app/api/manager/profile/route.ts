/**
 * GET  /api/manager/profile — the calling manager's OWN member profile.
 * PATCH /api/manager/profile — update the calling manager's OWN member profile.
 *
 * TIERED + self-scoped: gated via requireManager() (owner/admin only). A
 * manager can only ever read/write THEIR OWN CompanyMembership row — the
 * update is keyed on (companyId, userId=dbUserId from the resolved context),
 * never on a client-supplied id. A seller_member can't reach a manager
 * context, so this 403s for them; an admin can't edit another member's
 * profile because there's no path to address another row.
 *
 * Profile fields (displayName, title, bio, photoUrl, phone) mirror the seller
 * profile on SpaceSetting but live per-member on CompanyMembership — each
 * admin has their own profile within the company.
 */
