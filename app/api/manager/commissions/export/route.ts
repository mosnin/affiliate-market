...
/**
 * GET /api/manager/commissions/export
...
export async function GET(req: NextRequest) {
  ...
  // amount. Letting seller_member export the whole company's ledger is
  ...
  if (error) {
    console.error('[manager/commissions/export] query failed', error);
  ...
  for (const r of rows) {
    lines.push(
      [
        ...
        escapeCsv(r.managerRate),
        escapeCsv(r.managerAmount),
  ...
  // Company has no `slug` field in the current schema; use the id as a
  // stable, URL-safe identifier.
  const slug = ctx.company.id;
