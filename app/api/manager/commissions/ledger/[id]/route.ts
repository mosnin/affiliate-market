...
/**
 * PATCH /api/manager/commissions/ledger/[id]
 * Update a commission-ledger row. Only manager_owner / manager_admin may call.
 *
 * Body (any subset):
 *   { status?, payoutAt?, agentRate?, managerRate?, referralRate?,
 *     referralUserId?, notes? }
 *
 * Amounts (agentAmount / managerAmount / referralAmount) are recomputed from
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  let managerRateVal: number | undefined;
  if (hasManagerRate) {
    if (!isValidRate(body.managerRate)) {
      return NextResponse.json({ error: 'Invalid managerRate (must be 0-100)' }, { status: 400 });
    }
    managerRateVal = body.managerRate;
  }
  ...
  // Load the ledger row; must belong to this company.
  ...
  if (fetchErr) {
    console.error('[manager/commissions/ledger/PATCH] fetch failed', fetchErr);
  ...
  if (referralUserIdVal !== undefined && referralUserIdVal !== null) {
    const { data: userRow, error: userErr } = await supabase
      ...
      .maybeSingle();
    if (userErr) {
      console.error('[manager/commissions/ledger/PATCH] user lookup failed', userErr);
  ...
  const rateChanged = hasAgentRate || hasManagerRate || hasReferralRate;
  const effectiveAgentRate = agentRateVal ?? row.agentRate;
  const effectiveManagerRate = managerRateVal ?? row.managerRate;
  const effectiveReferralRate = referralRateVal ?? row.referralRate;
  const dealValue = row.dealValue;

  // Audit found: individually-capped rates (0-100 each) still allow a manager
  ...
  const rateSum =
    ...
    (effectiveManagerRate ?? 0) +
    (effectiveReferralRate ?? 0);
  if (rateSum > 100) {
    return NextResponse.json(
      {
        error: `Rates sum to ${rateSum}% — agent + manager + referral cannot exceed 100% of the deal value.`,
      },
      { status: 400 },
    );
  }
  ...
  if (managerRateVal !== undefined) updates.managerRate = managerRateVal;
  ...
  if (rateChanged) {
    ...
    updates.managerAmount = (dealValue * effectiveManagerRate) / 100;
    updates.referralAmount = (dealValue * effectiveReferralRate) / 100;
  }

  // Scope the update to companyId to guard against TOCTOU races.
  ...
  if (updateErr) {
    console.error('[manager/commissions/ledger/PATCH] update failed', updateErr);
