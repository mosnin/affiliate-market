/**
 * GET /api/manager/morning
 *
 * The manager's morning narration — ONE prioritized action the manager should
 ...
 * question the manager can answer with a tap.
 *
 * Priority ladder (descending urgency):
 *   1. stuck-deals      — highest (days × value) score across the team.
 *   2. unassigned-leads — company-tagged Contacts sitting in the queue.
 *   3. overloaded       — a seller whose open work (active deals + open
 *                         leads) exceeds 15 — the canonical team_health
 *                         threshold from agent/tools/manager/team.py.
 *   4. underused        — a seller with open work below 3 who's still
 *                         active in the last 30 days (canonical team_health
 *                         underused signal). Newly-invited dormant members
 *                         don't surface — only currently-active sellers
 *                         going quiet.
 ...
 *     stats: { sellers, activeDeals, gciMtd } }
 *
 * `suggestedPrompt` is non-null on branches 1-4 (the "want me to ..."
 * affordance); the component routes the manager into /manager/cola with the
 * prompt pre-filled. Branches 5 and 6 have no action, so the headline
 * renders non-interactive.
 *
 * The thresholds and definitions mirror agent/tools/manager/ exactly — the
 * Python tools are the canonical math for "what's happening on the team".
 * If they drift, Cola's narrated answer and this headline will disagree,
 * which is the worst kind of inconsistency.
 *
 * Auth: manager_owner or manager_admin. Seller-only members get 403 — they
 * have their own /cola morning story.
 */
...
export interface ManagerMorningResponse {
  /**
   * The single sentence the manager reads first thing. Formed as
   * "<fact> — <call-to-action>." so the component can render the
   * call-to-action in slightly stronger weight without parsing HTML.
   */
  headline: string;
  /**
   * Text to prefill into /manager/cola when the manager taps the headline.
...
// agent/tools/manager/pipeline.py:42 so the row pill and Cola answer the
// same question with the same math.
const STUCK_MIN_DAYS = 7;

// team_health canonical thresholds — see agent/tools/manager/team.py:55-57.
// Keep these in lockstep with the Python tool; the manager hears one number
// across the surface.
...
// the sellers page and team_health use to define "responding".
const RESPONSE_OUTBOUND_TYPES = ['call', 'email', 'meeting'] as const;

// GCI milestones the manager actually wants to hear about. Crossed-this-month
...
/** First name only — manager speaks about sellers the way they call them. */
...
interface SellerLoad {
  ...
  openLoad: number;
  /** True when the seller has any outbound activity in the last 30 days. */
...
export async function GET() {
  ...
  const memberRows: MemberRow[] = members
    .map((m) => ({
      userId: m.userId,
      name: m.User?.name ?? m.User?.email ?? 'Seller',
      spaceId: m.Space?.id ?? '',
    }))
  ...
  // Six queries; nothing fancier than what /manager/sellers already does.
  // Each is company-scoped through `spaceId IN (...)` or
  // `companyId = ...` so RLS doesn't leak across companies even if
  // someone bypasses requireManager().
  const [
    allActiveDealsRes,
    stuckCandidatesRes,
    openLeadsRes,
    ...
    // Every active deal — just spaceId for the per-seller active-deals count
    ...
    // Open leads per seller — Contact WHERE type='QUALIFICATION'. This is
    // the same definition team_health uses for "open_leads". The manager's
    ...
    // Unassigned company leads — same predicates as
    // app/manager/leads/page.tsx and find_unassigned_leads:
    // companyId match, tag includes 'company-lead', NOT 'assigned'.
    supabase
      .from('Contact')
      .select('id, tags')
  ...
  // ── Stuck deals: rank by (days × value), pick the top seller ───────────
  ...
  for (const [spaceId, rows] of stuckBySpace) {
    ...
    // applied per seller instead of per deal. Floor of $1 on value so a
  ...
  // ── Per-seller open load (active deals + open leads) ───────────────────
  ...
  const loads: SellerLoad[] = memberRows.map((m) => {
  ...
  // Overloaded = highest open_load AND open_load > 15. Single-seller team
  // never trips this — the manager can't redistribute to themselves.
  let overloaded: SellerLoad | null = null;
  if (loads.length >= 2) {
  ...
  // copy: we don't call out a seller who's already on vacation or hasn't
  // started. Same guard as team_health._load_signal.
  let underused: SellerLoad | null = null;
  if (loads.length >= 2) {
  ...
  // 2. Unassigned leads.
    ...
    headline = `${n} ${leadWord} sitting unassigned — want me to route ${n === 1 ? 'it' : 'them'} to your fewest-loaded seller?`;
    suggestedPrompt =
      n === 1
        ? 'Route the unassigned lead to the seller with the lightest load.'
        : `Route the ${n} unassigned leads to the seller with the lightest load.`;
  }
  // 3. Overloaded seller.
  else if (overloaded) {
    const fn = firstName(overloaded.name);
    // Fact framing — never "Bob is slow," always "Bob's at 18 active vs
    // threshold 15." Mirrors the row-pill voice from /manager/sellers.
  ...
  const response: ManagerMorningResponse = {
    headline,
    suggestedPrompt,
    stats: {
      sellers: memberRows.length,
