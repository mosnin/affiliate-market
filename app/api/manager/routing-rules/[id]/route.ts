...
/**
 * PATCH /api/manager/routing-rules/[id]
 *
 * Partial update restricted to manager_owner / manager_admin. The route
 * re-verifies the row belongs to the caller's company before writing,
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  if (loadErr) {
    logger.error(
      '[manager/routing-rules/PATCH] load failed',
      { ruleId, companyId: ctx.company.id },
      loadErr,
    );
  ...
  // new target is still a member of THIS company.
  if (
    ...
    if (memErr) {
      logger.error(
        '[manager/routing-rules/PATCH] destination membership check failed',
        { ruleId, companyId: ctx.company.id, newUserId },
        memErr,
      );
  ...
  if (updateErr) {
    logger.error(
      '[manager/routing-rules/PATCH] update failed',
      { ruleId, companyId: ctx.company.id },
      updateErr,
    );
...
/**
 * DELETE /api/manager/routing-rules/[id]
 *
 * Removes a rule. Restricted to manager_owner / manager_admin.
 */
export async function DELETE(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  if (deleteErr) {
    logger.error(
      '[manager/routing-rules/DELETE] delete failed',
      { ruleId, companyId: ctx.company.id },
      deleteErr,
    );
