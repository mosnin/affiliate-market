...
// ── GET — any manager member may read their company's rules ──────────────────

/**
 * GET /api/manager/routing-rules
 *
 * Returns the company's routing rules ordered by priority ASC then
 * createdAt ASC — the same order the engine evaluates them in. Any
 * company member can read (helpful for debugging / transparency into
...
export async function GET(): Promise<NextResponse> {
  ...
  if (error) {
    ...
    logger.error(
      '[manager/routing-rules/GET] list failed',
...
// ── POST — create a new rule (manager_owner / manager_admin only) ───────────────

/**
 * POST /api/manager/routing-rules
 *
 * Creates a rule for the current company. See the migration header for
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  // company — otherwise we'd accept cross-tenant IDs. We don't require
  // the target to be a seller_member (owners can legitimately want a
  // VIP rule that points at themselves) but we DO require company
  // membership.
  if (data.destinationUserId) {
    ...
    if (memErr) {
      logger.error(
        '[manager/routing-rules/POST] destination membership check failed',
        { companyId: ctx.company.id, destinationUserId: data.destinationUserId },
        memErr,
      );
    ...
    if (!membership) {
      return NextResponse.json(
        { error: 'destinationUserId is not a member of this company' },
        { status: 400 },
      );
    }
  }
  ...
  if (insertErr || !inserted) {
    logger.error(
      '[manager/routing-rules/POST] insert failed',
      { companyId: ctx.company.id },
      insertErr,
    );
