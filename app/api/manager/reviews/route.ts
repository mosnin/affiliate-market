...
/**
 * GET /api/manager/reviews?status=open|approved|closed|all
 * Default status = 'open'. Returns up to 200 rows belonging to this company.
...
export async function GET(req: NextRequest) {
  ...
  if (rowsErr) {
    logger.error('[manager/reviews/GET] list failed', { companyId: ctx.company.id }, rowsErr);
  ...
  if (usersRes.error) {
    logger.error('[manager/reviews/GET] user fetch failed', { companyId: ctx.company.id }, usersRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  if (dealsRes.error) {
    logger.error('[manager/reviews/GET] deal fetch failed', { companyId: ctx.company.id }, dealsRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  if (commentsRes.error) {
    logger.error('[manager/reviews/GET] comment count failed', { companyId: ctx.company.id }, commentsRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  ...
  if (spacesRes.error) {
    logger.error('[manager/reviews/GET] space fetch failed', { companyId: ctx.company.id }, spacesRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
