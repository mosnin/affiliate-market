...
import { getManagerMemberContext, requireManager } from '@/lib/permissions';
 ...
 * Returns a single review (scoped to the caller's company) plus its
 * comments in chronological order. Any manager member role may read.
 */
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const { data: row, error } = await supabase
    ...
    .select(
      'id, dealId, status, reason, createdAt, resolvedAt, resolvedByUserId, resolvedNote, requestingUserId, companyId',
    )
    .eq('id', reviewId)
  ...
  if (error) {
    logger.error(
      '[manager/reviews/GET] load failed',
      { reviewId, companyId: ctx.company.id },
      error,
    );
  ...
  try {
    ...
    logger.error('[manager/reviews/GET] shape failed', { reviewId }, err);
    return NextResponse.json({ error: 'Failed to load review' }, { status: 500 });
  }
}
 ...
 * Only manager_owner/manager_admin may resolve. Already-resolved rows return 409.
 */
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  // Role gate — requireManager already restricts to manager_owner/manager_admin,
  // but we double-check defensively so this route's contract doesn't silently
  // widen if requireManager's behavior ever changes.
  ...
  if (loadErr) {
    logger.error(
      '[manager/reviews/PATCH] load failed',
      { reviewId, companyId: ctx.company.id },
      loadErr,
    );
    return NextResponse.json({ error: 'Failed to load review' }, { status: 500 });
  ...
  if (updateErr) {
    logger.error('[manager/reviews/PATCH] update failed', { reviewId }, updateErr);
  ...
  try {
    ...
    logger.error('[manager/reviews/PATCH] shape failed', { reviewId }, err);
