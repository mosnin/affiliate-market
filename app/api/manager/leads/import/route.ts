...
export async function POST(req: NextRequest) {
  ...
  // ── Rate limit ──────────────────────────────────────────────────────────
  const { allowed } = await checkRateLimit(`manager-import:${company.id}`, 5, 3600);
  if (!allowed) {
  ...
  const colIdx = {
    ...
    address: findIndex(headers, ['product address', 'address', 'productaddress']),
  ...
  // Default space: the company owner's space
  const defaultSpaceId = spaceByOwner[company.ownerId] ?? null;
  if (!defaultSpaceId) {
    return NextResponse.json(
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    if (assignTo) {
      ...
      if (!targetUserId) {
        errors.push(`Row ${rowNum}: Member "${assignTo}" not found in company. Using default.`);
      } else {
    ...
    contactsToInsert.push({
      ...
      notes: notes || null,
      type: 'QUALIFICATION',
      tags: ['new-lead', 'imported', 'company-lead'],
