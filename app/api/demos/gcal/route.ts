...
/** POST — exchange OAuth code for tokens OR sync a specific demo */
export async function POST(req: NextRequest) {
  ...
  // Handle syncing a demo to Google Calendar
  if (action === 'sync_demo') {
    const { demoId } = body;
    ...
    const event = {
      summary: `Demo: ${demo.guestName}`,
      description: [
        `Guest: ${demo.guestName}`,
        demo.guestEmail ? `Email: ${demo.guestEmail}` : '',
        demo.guestPhone ? `Phone: ${demo.guestPhone}` : '',
        demo.productAddress ? `Product: ${demo.productAddress}` : '',
        demo.notes ? `Notes: ${demo.notes}` : '',
      ].filter(Boolean).join('\n'),
      start: { dateTime: demo.startsAt, timeZone: 'UTC' },
      end: { dateTime: demo.endsAt, timeZone: 'UTC' },
    };

    const calendarId = tokenRow.calendarId || 'primary';

    // Update existing event or create new
    let googleEventId = demo.googleEventId;
    if (googleEventId) {
