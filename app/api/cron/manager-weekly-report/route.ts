...
/**
 * GET /api/cron/manager-weekly-report
 *
 * Runs every Monday at 9 AM. Compiles a weekly activity report for each
 * active company and emails it to the manager owner.
 *
 ...
 * fire, cold-start race). Without it, every manager gets the report twice.
 */
async function handler(req: NextRequest) {
  const cronSecret = process.env.CRON_SECRET;
  if (!cronSecret) {
    console.error('[cron/manager-weekly-report] CRON_SECRET env var is not set — rejecting request');
  ...
  const lockKey = `cron-lock:manager-weekly-report:${today}`;
  ...
  try {
    ...
    const { data: companies, error: mgrErr } = await supabase
      .from('Company')
    ...
    if (!companies?.length) {
      return NextResponse.json({ sent: 0, message: 'No active companies' });
    }
    ...
    for (const company of companies) {
      try {
        ...
        const [newLeadsRes, contactedLeadsRes, newDealsRes, wonDealsRes] = await Promise.all([
          // New leads assigned (contacts created in the past 7 days with assigned-by-manager tag)
          supabase
            .from('Contact')
            .select('id, spaceId')
            .in('spaceId', spaceIds)
        ...
        for (const m of rawMembers) {
          ...
          const row: AgentRow = {
            ...
            email: user?.email ?? '',
            role: m.role === 'manager_owner' ? 'Owner' : m.role === 'manager_admin' ? 'Admin' : 'Seller',
        ...
        if (process.env.RESEND_API_KEY) {
          ...
          const html = buildWeeklyReportHtml({
            ...
            dashboardUrl: `${appUrl}/manager`,
          });

          const safeName = company.name.replace(/[\r\n\t]/g, ' ').slice(0, 100);
        ...
        console.error('[cron/manager-weekly-report] Failed for company', {
    ...
    console.error('[cron/manager-weekly-report] Unhandled error', error);
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden">
        ...
        <tr><td style="background:#0f172a;padding:20px 28px">
          <p style="margin:0;color:#94a3b8;font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.05em">${esc(companyName)}</p>
        ...
        <tr><td style="padding:16px 28px;border-top:1px solid #f1f5f9">
          <p style="margin:0;font-size:11px;color:#9ca3af;text-align:center">
            This weekly report is automatically generated for <strong>${esc(companyName)}</strong>. Manage your notification preferences in manager settings.
          </p>
        </td></tr>
      </table>
    </td></tr>
