...
export default async function JoinWithCodePage({ params }: Params) {
  ...
  if (!userId) {
    redirect(`/login/seller?redirect_url=/join/${code}`);
  }
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="bg-foreground px-6 py-5">
            <div className="flex items-center gap-2.5">
              {company?.logoUrl ? (
                <img src={company.logoUrl} alt="" className="h-6 max-w-[80px] object-contain rounded" />
              ) : (
            ...
            <p className="mt-1 text-background/60 text-sm">Company invite code</p>
          </div>
              ...
              <>
                <div>
                  <p className="text-sm text-foreground leading-relaxed">
                    ...
                    <span className="font-semibold">{company.name}</span> as a{' '}
                    <span className="font-semibold">Seller Member</span>.
                  </p>
