...
export function SignupForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    router.push(`/buyer/verify?email=${encodeURIComponent(email.trim().toLowerCase())}`);
  }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        <Link href="/buyer/reset" className="transition-colors hover:text-foreground">
...
export function ResetForm() {
  ...
  return (
    <form onSubmit={confirmReset} className="space-y-4">
      <p className="text-sm text-muted-foreground">
        If that email has an account, a code is on its way. Enter it below with a new password.
      </p>
      <Field label="6-digit code" htmlFor="code">
        <TextInput
          id="code"
