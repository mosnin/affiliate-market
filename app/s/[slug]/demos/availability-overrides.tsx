...
export function AvailabilityOverrides({ slug, productProfiles = [] }: AvailabilityOverridesProps) {
  ...
  const loadOverrides = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/demos/overrides?slug=${encodeURIComponent(slug)}`);
      if (res.ok) {
  ...
  async function handleSave() {
    ...
    setFormError(null);
    try {
      const res = await fetch('/api/demos/overrides', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          productProfileId: formProductId || null,
        }),
      });
      if (!res.ok) {
  ...
  async function handleDelete(id: string) {
    setDeletingId(id);
    try {
      const res = await fetch(`/api/demos/overrides/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const profileMap = new Map(productProfiles.map((p) => [p.id, p]));
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          ...
          {productProfiles.length > 0 && (
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Product (optional)</label>
              <select
                value={formProductId}
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {overrides.map((o) => {
            const profile = o.productProfileId ? profileMap.get(o.productProfileId) : null;
            return (
              <div
                key={o.id}
                className={cn(
