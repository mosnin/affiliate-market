...
interface ProductProfilesProps {
  ...
  onUpdate: (profiles: ProductProfile[]) => void;
}
...
export function ProductProfiles({ slug, profiles: initialProfiles, onUpdate }: ProductProfilesProps) {
  ...
  function startEdit(p: ProductProfile) {
    ...
    setFormDuration(p.demoDuration);
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        const res = await fetch(`/api/demos/products/${editingId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: formName.trim(),
            address: formAddress.trim() || null,
            demoDuration: formDuration,
            startHour: formStart,
            endHour: formEnd,
        ...
        const res = await fetch('/api/demos/products', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            slug,
  ...
  async function handleDelete(id: string) {
    setDeletingId(id);
    try {
      const res = await fetch(`/api/demos/products/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  async function toggleActive(id: string, isActive: boolean) {
    setTogglingId(id);
    try {
      const res = await fetch(`/api/demos/products/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        ...
        setFormError('Failed to update product status');
      }
    } catch {
      setFormError("Couldn't reach the server — usually temporary.");
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold">Product Profiles</h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            Each product can have its own demo hours, duration, and booking link.
          </p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Buffer Between Demos (min)</label>
      ...
      {profiles.length === 0 && !showForm ? (
        <div className="text-center py-10 text-muted-foreground space-y-2">
          ...
          <p className="text-sm">No product profiles yet</p>
          <p className="text-xs">Add products to give each listing its own demo schedule and booking link.</p>
        </div>
      ) : (
          ...
          {profiles.map((p) => (
            <div key={p.id} className={cn(
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button onClick={() => toggleActive(p.id, !p.isActive)} disabled={togglingId === p.id} className={cn('w-7 h-7 flex items-center justify-center rounded-md hover:bg-muted transition-colors disabled:opacity-50', p.isActive ? 'text-positive dark:text-positive' : 'text-muted-foreground')} title={p.isActive ? 'Deactivate' : 'Activate'}>
