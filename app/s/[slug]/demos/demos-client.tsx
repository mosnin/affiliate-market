...
import { DemoAvailabilityManager } from './demo-availability-manager';
import { DemoPrepCard } from '@/components/demos/demo-prep-card';
import { DemoTimeline } from '@/components/demos/demo-timeline';
import { DemoStatsStrip } from '@/components/demos/demo-stats-strip';
import { DemoFeedbackBadge } from '@/components/demos/demo-feedback-badge';
import {
  H1,
  H2,
  H3,
...
type DemoStatus = 'scheduled' | 'confirmed' | 'completed' | 'cancelled' | 'no_show';
...
interface DemoSettings {
  demoDuration: number;
  demoBufferMinutes: number;
  demoStartHour: number;
  demoEndHour: number;
  demoDaysAvailable: number[];
  demoBlockedDates: string[];
}
...
export function DemosClient({ slug, spaceId, initialDemos, hasGoogleCalendar, bookingUrl, productProfiles: initialProfiles = [], demoSettings: initialDemoSettings }: DemosClientProps) {
  ...
  // --- Supabase Realtime: keep demos in sync across tabs / devices ---
  useRealtime<Record<string, unknown>>({
    table: 'Demo',
    filter: `spaceId=eq.${spaceId}`,
    onEvent: (payload) => {
      if (payload.eventType === 'INSERT') {
        const newDemo = payload.new as unknown as Demo;
        setDemos((prev) => {
          // Avoid duplicates (e.g. if we just created it locally)
          if (prev.some((t) => t.id === newDemo.id)) return prev;
          return [newDemo, ...prev];
        });
      } else if (payload.eventType === 'UPDATE') {
        const updated = payload.new as unknown as Demo;
        setDemos((prev) =>
        ...
        if (deleted?.id) {
          setDemos((prev) => prev.filter((t) => t.id !== deleted.id));
        }
      }
    },
  });
  ...
  const filtered = demos.filter((t) => {
    ...
    if (searchLower) {
      return (
        ...
        (t.productAddress?.toLowerCase().includes(searchLower) ?? false) ||
  ...
  const updateStatus = useCallback(async (demoId: string, status: DemoStatus) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDemos((prev) => prev.map((t) => (t.id === demoId ? { ...t, ...updated } : t)));
      } else {
        const data = await res.json().catch(() => ({}));
        console.error('[Demos] Status update failed:', data.error);
      }
    } catch (err) {
      console.error('[Demos] Status update error:', err);
    } finally {
      setUpdatingId(null);
    }
  }, []);

  const syncToGcal = useCallback(async (demoId: string) => {
    setSyncingId(demoId);
    try {
      const res = await fetch('/api/demos/gcal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slug, action: 'sync_demo', demoId }),
      });
      if (res.ok) {
        const data = await res.json();
        setDemos((prev) =>
          prev.map((t) => (t.id === demoId ? { ...t, googleEventId: data.googleEventId } : t))
        );
      }
    } catch (err) {
      console.error('[Demos] Sync failed:', err);
    } finally {
      setSyncingId(null);
    }
  }, [slug]);

  const convertToDeal = useCallback(async (demoId: string) => {
    setConvertingId(demoId);
    try {
      const res = await fetch('/api/demos/convert', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slug, demoId }),
      });
      if (res.ok) {
        const data = await res.json();
        setDemos((prev) =>
          prev.map((t) => (t.id === demoId ? { ...t, sourceDealId: data.deal.id } : t))
        );
      ...
      console.error('[Demos] Convert failed:', err);
    } finally {
  ...
  return (
    <div className={PAGE_RHYTHM}>
      ...
      {/* Stats */}
      <DemoStatsStrip demos={demos.map((t) => ({ startsAt: t.startsAt, status: t.status, sourceDealId: t.sourceDealId, createdAt: t.createdAt }))} />
          ...
          <div className="border-t border-border pt-6 space-y-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Paste this code into your website or listing page to let visitors book demos directly.
              </p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border bg-card">
                  {filtered.map((demo) => {
                    const { date, time } = formatDateTime(demo.startsAt);
                    const endTime = formatDateTime(demo.endsAt).time;
                    const dur = getDuration(demo.startsAt, demo.endsAt);
                    const statusConf = STATUS_CONFIG[demo.status];
                    const isPast = new Date(demo.startsAt) < now;
                    ...
                    return (
                      <tr
                        ...
                        className={cn(
                          ...
                          isPast && demo.status !== 'completed' && 'opacity-70',
                        )}
                      >
                        <td className="px-4 py-3">
                          <div>
                            <p className="font-medium">{demo.guestName}</p>
                            <p className="text-xs text-muted-foreground">{demo.guestEmail}</p>
                            {demo.guestPhone && (
                              <p className="text-xs text-muted-foreground sm:hidden">{demo.guestPhone}</p>
                            )}
                        ...
                        <td className="px-4 py-3 hidden lg:table-cell text-xs">
                          {demo.Contact ? (
                            <a
                              href={`/s/${slug}/contacts/${demo.Contact.id}`}
                        ...
                        <td className="px-4 py-3">
                          <div className="relative">
                            <button
                              onClick={() => setActionMenuId(actionMenuId === demo.id ? null : demo.id)}
                              disabled={updatingId === demo.id || convertingId === demo.id}
                              className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-muted transition-colors disabled:opacity-50"
                            >
                              {updatingId === demo.id ? <Loader2 size={14} className="animate-spin" /> : <MoreHorizontal size={14} />}
                            </button>
                            {actionMenuId === demo.id && (
                              <div className="absolute right-0 top-8 z-20 w-40 rounded-lg border border-border bg-card shadow-lg dark:shadow-none py-1">
                                {demo.status === 'scheduled' && (
                                  <button onClick={() => updateStatus(demo.id, 'confirmed')} className="w-full text-left px-3 py-1.5 text-xs hover:bg-accent transition-colors">
                                    Confirm
                                  </button>
                                )}
                                {(demo.status === 'scheduled' || demo.status === 'confirmed') && (
                                  <>
                                    <button onClick={() => updateStatus(demo.id, 'completed')} className="w-full text-left px-3 py-1.5 text-xs hover:bg-accent transition-colors">
                                      Mark Completed
                                    </button>
                                    <button onClick={() => updateStatus(demo.id, 'no_show')} className="w-full text-left px-3 py-1.5 text-xs hover:bg-accent transition-colors">
                                      Mark No-Show
                                    </button>
                                    <button onClick={() => updateStatus(demo.id, 'cancelled')} className="w-full text-left px-3 py-1.5 text-xs text-destructive hover:bg-accent transition-colors">
                                      Cancel Demo
                                    </button>
                                  </>
                                )}
                                {(demo.status === 'completed' || demo.status === 'confirmed') && !demo.sourceDealId && (
                                  <button
                                    onClick={() => { setActionMenuId(null); convertToDeal(demo.id); }}
                                    className="w-full text-left px-3 py-1.5 text-xs hover:bg-accent transition-colors flex items-center gap-1.5"
                                  >
                                    <Briefcase size={11} />
                                    {convertingId === demo.id ? 'Creating...' : 'Create Deal'}
                                  </button>
                                )}
                                {demo.sourceDealId && (
                                  <a
                                    href={`/s/${slug}/deals/${demo.sourceDealId}`}
                                ...
                                {demo.status === 'cancelled' && (
                                  <button onClick={() => updateStatus(demo.id, 'scheduled')} className="w-full text-left px-3 py-1.5 text-xs hover:bg-accent transition-colors">
                                    Reschedule
                                  </button>
                                )}
                              </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium truncate">{demo.guestName}</span>
                    ...
                    <div className="flex items-center gap-3">
                      <DemoTimeline
                        ...
                        googleEventId={demo.googleEventId}
                        sourceDealId={demo.sourceDealId}
                      />
                      <DemoFeedbackBadge demoId={demo.id} slug={slug} status={demo.status} />
                    </div>
                  </div>

                  {/* Actions */}
                  <div className="flex items-center gap-1.5 sm:flex-shrink-0 relative">
                    {!isPast && demo.status !== 'cancelled' && (
                      <DemoPrepCard demoId={demo.id} />
                    )}
                    {hasGoogleCalendar && !demo.googleEventId && demo.status !== 'cancelled' && (
                      <button
                        onClick={() => syncToGcal(demo.id)}
                        disabled={syncingId === demo.id}
                      ...
                      >
                        {syncingId === demo.id ? <Loader2 size={12} className="animate-spin" /> : <CalendarPlus size={12} />}
                        Sync
                      </button>
                    )}
                    {demo.googleEventId && (
