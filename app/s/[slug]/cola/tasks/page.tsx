...
export default async function AgentTasksPage({
  ...
  return (
    <ColaPageShell
      greeting="Tasks."
      title="Agent Tasks"
      subtitle="Long-running goals Cola is working on."
    >
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <ArrowLeft size={12} /> Cola
        </Link>
        <Link
          href={`/s/${slug}/cola/approvals`}
      ...
      {taskList.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-6 py-10 text-center">
          <p className="text-sm text-muted-foreground">
            No tasks yet. Ask Cola to work on a complex goal and it will appear here.
          </p>
        </div>
      ) : (
        <ul className="divide-y divide-border/60">
          {taskList.map((task) => {
            ...
            return (
              <li key={task.id}>
                <Link
                  href={`/s/${slug}/cola/tasks/${task.id}`}
