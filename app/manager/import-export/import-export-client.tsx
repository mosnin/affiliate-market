...
export default function ImportExportClient({ totalLeads }: { totalLeads: number }) {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const res = await fetch('/api/manager/leads/export');
      if (!res.ok) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/manager/leads/import', {
        method: 'POST',
        body: formData,
      });
  ...
  const subtitle = totalLeads
    ? `${totalLeads.toLocaleString()} ${totalLeads === 1 ? 'lead' : 'leads'} across the company.`
...
{`Name,Email,Phone,Lead Type,Budget,Product Address,Notes,Move-in Date,Assign To
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <ul className="divide-y divide-border/60">
              ...
              <li className="py-2 text-xs">
                <span className="font-medium text-foreground">Product Address</span>
              </li>
              <li className="py-2 text-xs">
