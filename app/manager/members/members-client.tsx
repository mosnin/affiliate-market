...
import { RemoveMemberButton } from '@/components/manager/remove-member-button';
import { ChangeRoleButton } from '@/components/manager/change-role-button';
import { OffboardMemberDialog } from '@/components/manager/offboard-member-dialog';
...
export function MembersClient({
  ...
  // One-sentence status read of the team. Loud fact wins. Company name
  // anchors the empty case so the page still reads as "your team."
  const subtitle = (() => {
    if (members.length === 0) return `Nobody on the team at ${companyName} yet.`;
    if (pendingCount > 0) {
        ...
        <>
          ...
          {filtered.length === 0 ? (
            ...
            <ul className="divide-y divide-border/60">
              {filtered.map((m) => {
                const canManage =
                  m.role !== 'manager_owner' &&
                  (currentUserRole === 'manager_owner' ||
                    (currentUserRole === 'manager_admin' && m.role === 'seller_member'));
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 min-w-0 flex-wrap">
                        ...
                        {!m.userOnboard && (
                          <span className="inline-flex text-[10px] font-semibold rounded-full px-2 py-0.5 flex-shrink-0 text-muted-foreground bg-muted dark:text-muted-foreground dark:bg-muted0/15">
                            Pending
                          </span>
                        )}
                      </div>
