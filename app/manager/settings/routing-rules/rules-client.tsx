...
export default function RulesClient({
  ...
  const reloadList = useCallback(async () => {
    try {
      const res = await fetch('/api/manager/routing-rules', { cache: 'no-store' });
      if (res.ok) {
  ...
  const handleToggleEnabled = useCallback(
    async (rule: DealRoutingRuleRow) => {
      ...
      try {
        const res = await fetch(`/api/manager/routing-rules/${rule.id}`, {
          method: 'PATCH',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? `/api/manager/routing-rules/${mode.rule.id}` : '/api/manager/routing-rules',
        {
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                <div>
                  ...
                  <p className={CAPTION}>
                    Uses the seller pool. Tag narrowing is ignored until the membership tags column ships.
                  </p>
                </div>
              </label>
            </div>
            ...
            {form.destinationKind === 'agent' ? (
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    {members.length === 0 ? (
                      <div className={cn(CAPTION, 'px-3 py-2')}>No company members yet.</div>
                    ) : (
                      members.map((m) => (
                        <SelectItem key={m.userId} value={m.userId}>
                          {m.name ?? m.email}
                          {m.role !== 'seller_member' ? ` · ${roleLabel(m.role)}` : ''}
                        </SelectItem>
                      ))
                    )}
                  </SelectContent>
...
function roleLabel(role: string): string {
  if (role === 'manager_owner') return 'Owner';
  if (role === 'manager_admin') return 'Admin';
  if (role === 'seller_member') return 'Seller';
  return role;
}
