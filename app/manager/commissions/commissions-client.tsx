...
// Company DEFAULT rates are stored as DECIMALS (0.03 = 3%); convert to
...
function buildEditDefaults(row: LedgerRow): EditFields {
  return {
    ...
    managerRate: formatPct(row.managerRate),
...
function EditRowDialog({ row, onClose, onSaved }: EditDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const newManager = parsePct(fields.managerRate);
    if (newManager === null) {
      toast.error('Manager rate must be between 0 and 100');
      return;
    }
    if (Math.abs(newManager - row.managerRate) > 1e-9) patch.managerRate = newManager;
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/manager/commissions/ledger/${row.id}`, {
        method: 'PATCH',
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="manager-rate">Manager %</Label>
              <Input
                id="manager-rate"
                type="number"
                step="0.01"
                min={0}
                max={100}
                value={fields.managerRate}
                onChange={(e) => setFields((p) => ({ ...p, managerRate: e.target.value }))}
              />
            </div>
            <div className="space-y-1.5">
...
function normalizeRow(apiResponse: unknown, prev: LedgerRow): LedgerRow {
  ...
  return {
    ...
    managerRate: pick('managerRate', prev.managerRate),
    ...
    managerAmount: pick('managerAmount', prev.managerAmount),
...
export function CommissionsClient({ ledger: initialLedger, defaultAgentRate, defaultManagerRate }: Props) {
  ...
  const summary = useMemo(() => {
    ...
    for (const r of filtered) {
      ...
      totalCommissions += r.managerAmount + r.agentAmount + r.referralAmount;
      if (r.status === 'paid') paid += r.managerAmount;
      else if (r.status === 'pending') pending += r.managerAmount;
    }
    return {
      totalCommissions,
      totalValue,
  ...
  const byAgent = useMemo(() => {
    ...
    for (const r of filtered) {
      ...
      if (existing) {
        ...
        existing.managerCommission += r.managerAmount;
        ...
        map.set(r.agentUserId, {
          ...
          managerCommission: r.managerAmount,
          rows: [r],
        });
      }
    }
  ...
  function handleExport() {
    ...
    const url = `/api/manager/commissions/export?${params.toString()}`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="text-xs text-muted-foreground pb-2">
            Defaults: agent {decimalToPct(defaultAgentRate)}% &middot; manager {decimalToPct(defaultManagerRate)}%
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-px rounded-xl overflow-hidden border border-border/60 bg-border/60">
        ...
        <div className="bg-background px-4 py-4">
          ...
          <p className={cn(CAPTION, 'mt-1')}>Pending (manager)</p>
        </div>
        <div className="bg-background px-4 py-4">
          <p className={cn(STAT_NUMBER)}>{formatCurrency(summary.paidThisMonth)}</p>
          <p className={cn(CAPTION, 'mt-1')}>Paid (manager)</p>
        </div>
        <div className="bg-background px-4 py-4">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="text-right px-4 py-3 text-xs font-medium text-muted-foreground">Manager earned</th>
              </tr>
            </thead>
              ...
              {byAgent.map((a) => (
                <tr key={a.agentUserId} className="hover:bg-foreground/[0.04] transition-colors">
                  ...
                  <td className="px-4 py-3 text-right tabular-nums text-xs font-medium">
                    {formatCurrency(a.managerCommission)}
                  </td>
                </tr>
              ))}
              {byAgent.length === 0 && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="text-right px-3 py-3 text-xs font-medium text-muted-foreground">Manager %</th>
                <th className="text-right px-3 py-3 text-xs font-medium text-muted-foreground">Manager $</th>
            ...
            <tbody className="divide-y divide-border/60 bg-card">
              {filtered.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-right tabular-nums text-xs">{formatPct(row.managerRate)}%</td>
                    <td className="px-3 py-3 text-right tabular-nums text-xs font-medium">
                      {formatCurrency(row.managerAmount)}
                    </td>
      ...
      <p className="text-xs text-muted-foreground">
        ...
        in effect at that moment; changes to company defaults only affect future
        won deals.
      </p>
