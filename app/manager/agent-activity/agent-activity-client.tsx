...
export interface ResponseShape {
  ...
  company: { totals: SellerRollup['totals']; sellerCount: number };
}
...
export function AgentActivityClient({ initial, companyName }: Props) {
  void companyName;
  ...
  function selectWindow(next: Window) {
    ...
    startTransition(async () => {
      try {
        const res = await fetch(`/api/manager/agent-activity?days=${next}`, { cache: 'no-store' });
  ...
  const headlineNumber = data.company.totals.all;
  const sellerCount = data.company.sellerCount;
  ...
  // seller's side. Routes collapse into one bucket so we don't overweight
  // company-only signal in the headline strip.
  const headlineCells = useMemo(() => {
    const t = data.company.totals;
    return [
      { label: 'Drafts', value: t.drafts, icon: FilePen },
      { label: 'Demos', value: t.demos, icon: CalendarCheck },
    ...
    <div className="space-y-8">
        ...
        >
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">
              {headlineNumber === 0
                ? <>Nothing yet {periodCopy(windowDays)}. Cola takes a few days to warm up on a new team.</>
                : <>actions {periodCopy(windowDays)} across {sellerCount} {sellerCount === 1 ? 'seller' : 'sellers'}.</>
              }
            </p>
          </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-3 pb-3 border-b border-border/60">
          ...
          {sellerCount > 0 && (
            <span className="text-[11px] text-muted-foreground tabular-nums">{sellerCount}</span>
          )}
          {pending && (
        ...
        {data.sellers.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Invite a seller and Cola&apos;s work will start landing here.
            </p>
          </div>
        ) : (
          <AnimatePresence mode="wait" initial={false}>
            <StaggerList key={`list-${windowDays}`} className="divide-y divide-border/60">
              {data.sellers.map((r) => (
                <StaggerItem key={r.spaceId}>
                  <SellerRow row={r} maxAll={data.sellers[0]?.totals.all ?? 0} />
                </StaggerItem>
              ))}
            </StaggerList>
          </AnimatePresence>
...
function SellerRow({ row, maxAll }: { row: SellerRollup; maxAll: number }) {
  const name = row.name ?? row.email ?? 'Unnamed';
  const t = row.totals;
  // Visual weight: a thin horizontal bar inside the row whose length is
  // proportional to total activity vs. the busiest seller in the window.
