...
// Action badge palette. Tuned to match existing manager pages — muted greys for
...
export function ActivityClient({ initialRows, initialCursor, actors, spaceMap, role }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const res = await fetch(`/api/manager/activity?${buildQuery()}`, { cache: 'no-store' });
  ...
  const handleLoadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await fetch(`/api/manager/activity?${buildQuery(cursor)}`, {
        cache: 'no-store',
      });
...
function EmptyState({ hasSearch, windowLabel }: { hasSearch: boolean; windowLabel: string }) {
  // Audit-driven copy: tell the manager they're searching a scoped window,
