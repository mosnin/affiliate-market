...
export interface AssignedLeadProgress {
  ...
  currentStage: 'QUALIFICATION' | 'DEMO' | 'APPLICATION';
...
function LeadNotes({ contactId }: { contactId: string }) {
  ...
  const loadNotes = useCallback(async () => {
    if (loaded) return;
    try {
      const res = await fetch(`/api/manager/lead-note?contactId=${contactId}`);
      if (res.ok) {
  ...
  async function handleAddNote(noteText: string) {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/manager/lead-note', {
        method: 'POST',
...
function UnassignedRow({
  ...
  lead: LeadRow;
  sellers: SellerOption[];
  onAssigned: (leadId: string, seller: SellerOption) => void;
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      const res = await fetch(`/api/manager/leads/${lead.id}`, { method: 'DELETE' });
  ...
  async function handleAssign(seller: SellerOption) {
    setAssigning(true);
    try {
      const res = await fetch('/api/manager/assign-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contactId: lead.id, sellerUserId: seller.userId }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "I couldn't route that lead.");
      }
      toast.success(`Routed to ${seller.name ?? seller.email}.`);
      onAssigned(lead.id, seller);
    } catch (err: unknown) {
    ...
    <li>
      <div className="group/row flex items-center gap-3 py-3 px-2 -mx-2 rounded-md hover:bg-muted/30 transition-colors">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <SellerPicker sellers={sellers} onSelect={handleAssign} disabled={assigning} />
          {confirmDelete ? (
...
function AssignedRow({
  ...
  onUnassigned?: (leadId: string, sellerName: string) => void;
}) {
  ...
  const sellerName = progress?.sellerName ?? lead.assignedTo ?? 'this seller';

  async function handleUnassign() {
    setUnassigning(true);
    try {
      const res = await fetch('/api/manager/unassign-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      toast.success(`Pulled back from ${sellerName}.`);
      setConfirmOpen(false);
      onUnassigned?.(lead.id, sellerName);
    } catch (err: unknown) {
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {lead.name || 'This lead'} comes off {sellerName}&rsquo;s plate and lands back in the unassigned queue.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <button
        ...
        <div className="pb-3 pl-[calc(32px+0.75rem)]">
          <div className="rounded-lg bg-muted/30 border border-border/60 p-3 space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div>
                <p className={SECTION_LABEL}>Seller</p>
                <p className="mt-0.5 font-medium text-foreground">{progress.sellerName}</p>
              </div>
              <div>
...
export function ManagerLeadsClient({
  unassignedLeads,
  assignedLeads,
  sellers,
  ...
  // Resolve the seller a routed lead sits with — same rule AssignedRow uses
  // (live progress wins, falls back to the assignment label).
  const sellerOf = useCallback(
    (lead: LeadRow) => assignedLeadProgress[lead.id]?.sellerName ?? lead.assignedTo ?? null,
    [assignedLeadProgress],
  );

  // Sellers who actually have routed leads, plus any team member — so the
  // menu mirrors the team even before everyone has a lead. Sorted, deduped.
  const sellerNames = useMemo(() => {
    const set = new Set<string>();
    for (const lead of assigned) {
      const name = sellerOf(lead);
  ...
  const filteredUnassigned = useMemo(() => {
    // A specific seller has no claim on unassigned leads — hide them.
    if (sellerFilter !== 'all') return [];
    return applySearch(unassigned);
  }, [unassigned, applySearch, sellerFilter]);

  const filteredAssigned = useMemo(() => {
    const bySeller =
      sellerFilter === 'all'
        ? assigned
        : assigned.filter((l) => sellerOf(l) === sellerFilter);
    return applySearch(bySeller);
  }, [assigned, applySearch, sellerFilter, sellerOf]);

  function handleAssigned(leadId: string, seller: SellerOption) {
    ...
    setAssigned((prev) => [
      {
        ...lead,
        assignedTo: seller.name ?? seller.email,
  ...
  return (
    <div className="space-y-8 pb-56 md:pb-24">
      {/* Search + by-seller filter — the only chrome. View toggles, sort
          dropdown, lead type filter, score pills, and "Add Lead" button all
          cut. Managers don't manually add leads; intake forms do. */}
      {hasAnyLeads && (
      ...
      {/* Fresh-company empty state — no leads at all. Canonical dashed card,
          one sentence, first-person Cola voice. */}
      {!hasAnyLeads && (
      ...
      {/* Unassigned — the one demanding routing. Shown first so it's the
          first thing the manager reaches for. Hidden when filtering to a
          single seller: unassigned leads belong to no one, so an empty
          queue under a seller filter is noise. */}
      {hasAnyLeads && sellerFilter === 'all' && (
        <section className="space-y-2">
          <SectionHeader
            label="Unassigned"
            count={filteredUnassigned.length}
            hint={filteredUnassigned.length > 0 ? 'Route to a seller' : undefined}
          />
            ...
            <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
              <p className="text-sm text-foreground">
                {isSearching
                  ...
                  : sellerFilter !== 'all'
                    ? `Nothing on ${sellerFilter}’s plate yet.`
                    : "Nothing on a seller's plate yet."}
              </p>
            </div>
          ) : (
            <ul className="divide-y divide-border/60">
