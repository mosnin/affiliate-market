/**
 * /manager/usage — Company AI spend aggregated across all member sellers.
 *
 * Three sections:
 *   1. Totals strip  — total tokens, total cost, total turns (this month).
 *   2. Per-seller   — divide-y breakdown sorted by cost desc. This is the
 *                       company-specific value: who spent what.
 *   3. Per-provider  — mirrors the seller UsageSection: provider label,
 *                       total tokens, cost, cache hit rate where available.
 *
 * Scoping: all CompanyMembership spaces (owner + admins + seller_members),
...
export const metadata: Metadata = { title: 'Usage — Company' };
...
export default async function ManagerUsagePage() {
  ...
  // "This month" is the natural window a manager thinks in (billing cycles,
  ...
  const sellerMap = new Map<string, SellerRollup>();
  ...
  for (const r of rows) {
    ...
    // Per-seller rollup
    const existing = sellerMap.get(r.spaceId) ?? {
      spaceId: r.spaceId,
    ...
    existing.turns += 1;
    sellerMap.set(r.spaceId, existing);
  ...
  // Sort sellers by cost desc.
  const sellers: SellerRollup[] = Array.from(sellerMap.values()).sort(
  ...
  const statusSentence = (() => {
    if (rows.length === 0) return `No Cola usage recorded for ${monthName} yet.`;
    const parts: string[] = [];
    parts.push(`${fmtCost(totalCost)} spent across ${sellers.length} ${sellers.length === 1 ? 'seller' : 'sellers'}`);
      ...
      {isEmpty ? (
        ...
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Cola activity across your member sellers will appear here.
          </p>
        </div>
      ) : (
        <div className={SECTION_RHYTHM}>
          ...
          <section className="space-y-3">
            <p className={SECTION_LABEL}>By seller</p>
            ...
            <p className={cn(META, 'text-right pt-1')}>
              {sellers.length} {sellers.length === 1 ? 'seller' : 'sellers'} · {monthName}
            </p>
          </section>
