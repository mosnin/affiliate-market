/**
 * /manager/analytics -- Company conversion funnel + per-agent breakdown.
 *
 * Shows the rental-funnel metrics aggregated across all member spaces.
 * Gated with resolveManagerContext() (manager_owner + manager_admin only).
 *
 * Structure:
...
export const metadata: Metadata = { title: 'Analytics -- Company' };
...
export default async function ManagerAnalyticsPage() {
  // Gate: manager_owner + manager_admin only.
  ...
  const agentData: AgentFunnelData[] = members
    ...
    .map((m) => {
      ...
      return {
        ...
        totalDeals,
        wonValue: s.wonValue,
        leadToDemo: s.totalLeads > 0 ? Math.round((s.demo / s.totalLeads) * 100) : 0,
        demoToApp: s.demo > 0 ? Math.round((s.application / s.demo) * 100) : 0,
      ...
      {isEmpty ? (
        ...
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Lead and deal data across your member sellers will appear here.
          </p>
        </div>
      ) : (
