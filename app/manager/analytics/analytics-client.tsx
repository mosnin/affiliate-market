...
/**
 * AnalyticsClient — interactive funnel + table breakdown for the company
 * analytics page.
 *
  ...
  return (
    <section className="rounded-xl border border-border/70 bg-background px-4 py-4 space-y-3">
      ...
      <div className="space-y-1.5 pt-1">
        ...
        <StepRate pct={agent.leadToDemo} />
        <FunnelBar label="Demos" value={agent.demos} maxValue={max} />
        <StepRate pct={agent.demoToApp} />
      ...
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-border/60">
        <div className="text-center">
          <p className="text-xs tabular-nums font-medium text-foreground">{agent.leadToDemo}%</p>
          <p className={cn(CAPTION, 'mt-0.5')}>Lead to demo</p>
        </div>
        <div className="text-center">
          <p className="text-xs tabular-nums font-medium text-foreground">{agent.demoToApp}%</p>
          <p className={cn(CAPTION, 'mt-0.5')}>Demo to app</p>
        </div>
        <div className="text-center">
...
export function AnalyticsClient({ agents }: Props) {
  ...
  const teamTotals = useMemo(() => {
    const t = agents.reduce(
      ...
      { totalLeads: 0, demos: 0, applications: 0, totalDeals: 0, wonDeals: 0, wonValue: 0 },
    );
    return {
      ...t,
      leadToDemo: t.totalLeads > 0 ? Math.round((t.demos / t.totalLeads) * 100) : 0,
      demoToApp: t.demos > 0 ? Math.round((t.applications / t.demos) * 100) : 0,
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-border/70 bg-background px-4 sm:px-5 py-5 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <StepRate pct={teamTotals.leadToDemo} />
          <FunnelBar label="Demos" value={teamTotals.demos} maxValue={teamTotals.totalLeads} />
          <StepRate pct={teamTotals.demoToApp} />
        ...
        sorted.length > 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Lead activity will appear here once sellers are active.
            </p>
          </div>
        )
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/60">
                ...
                <th className={cn(SECTION_LABEL, 'text-right px-3 py-2')}>Demos</th>
            ...
            <tbody className="divide-y divide-border/60">
              ...
              {sorted.map((a) => (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{a.demos}</td>
                  ...
                  {/* Rate columns */}
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{a.leadToDemo}%</td>
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground hidden md:table-cell">{a.demoToApp}%</td>
              ...
              {sorted.length > 0 && (
                <tr className="bg-foreground/[0.04]">
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{teamTotals.demos}</td>
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground">{teamTotals.leadToDemo}%</td>
                  <td className="px-3 py-3 text-right tabular-nums text-sm text-muted-foreground hidden md:table-cell">{teamTotals.demoToApp}%</td>
