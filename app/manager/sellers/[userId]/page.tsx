...
export const metadata: Metadata = { title: 'Seller Detail — Teams' };

type Params = { params: Promise<{ userId: string }> };

export default async function SellerDrilldownPage({ params }: Params) {
  ...
  if (!membership) redirect('/manager/sellers');
  ...
  if (!user) redirect('/manager/sellers');
  ...
  const roleLabel = membership.role === 'manager_owner' ? 'Owner' : membership.role === 'manager_admin' ? 'Admin' : 'Seller';
      ...
      {!space ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            This seller hasn&apos;t set up their workspace.
          </p>
        </div>
      ) : (
