...
export interface SellerRow {
  ...
  openLeads: number;
  /** Manager-assigned leads with no first contact (un-worked). */
  unworked: number;
  ...
  /** Band relative to team median (mirrors agent/tools/manager/performance.py). */
...
// Health sort order — needs-attention surfaces first so the manager acts.
const HEALTH_ORDER: Record<SellerRow['health'], number> = {
  'needs-attention': 0,
  'on-track':        1,
  'top-performer':   2,
  'pending':         3,
...
const roleLabel = (role: string) =>
  role === 'manager_owner' ? 'Owner' : role === 'manager_admin' ? 'Admin' : 'Seller';
...
function HealthChip({ health }: { health: SellerRow['health'] }) {
  const base =
...
function HealthNumbers({ r }: { r: SellerRow }) {
...
// ── Coach prompt ─────────────────────────────────────────────────────────────
// One-line health summary for the Cola prefill. Calm, factual, no em dashes.
function coachHealthSummary(r: SellerRow): string {
...
function buildCoachHref(r: SellerRow): string {
  ...
  return `/manager?prompt=${encodeURIComponent(prompt)}`;
}

export function SellersClient({ sellers }: { sellers: SellerRow[] }) {
            ...
            return (
              ...
              >
                ...
                <Link
                  ...
                  aria-label={`Coach ${displayName} with Cola`}
