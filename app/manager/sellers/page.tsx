...
import { SellersClient, type SellerRow } from './sellers-client';

export const metadata: Metadata = { title: 'Sellers — Teams' };
...
export default async function ManagerSellersPage() {
  ...
  const members = await getCompanyMembers(company.id, { includeOnboard: true, includeSpaceName: true });

  const spaceIds = members.map((m) => m.Space?.id).filter(Boolean) as string[];

  // 7-day response-time window. Mirrors agent/tools/manager/performance.py
  // (which uses 30 days for the deeper Cola audit); the inline row pill
  ...
  // Pull the full health picture per seller in one parallel volley:
  ...
  //   - unworkedRows      → manager-assigned leads with no first contact
  //   - slaNudgedRows     → leads that have hit the SLA nudge threshold
  //   - slaEscalatedRows  → leads that have escalated to the manager
  ...
  const [
    ...
    spaceIds.length > 0
      ? supabase
          ...
          .then((r) => r.data ?? [])
      : Promise.resolve([]),
    // Un-worked manager-assigned leads: assigned but lastContactedAt IS NULL
    spaceIds.length > 0
      ? supabase
          .from('Contact')
          .select('spaceId')
  ...
  // ContactActivity instead of N per-space queries — the manager page already
  ...
  // Team median across ALL response samples — same definition as the Cola
  ...
  // Band multipliers — match agent/tools/manager/performance.py:48-49 so
  // Cola's narrative answer ("Alice is slow") and the row pill agree.
  const FAST_MULTIPLIER = 0.5;
  const SLOW_MULTIPLIER = 2.0;
  function bandFor(avgHours: number | null): SellerRow['responseBand'] {
  ...
  const sellers: SellerRow[] = members.map((m) => {
  ...
  const subtitle = (() => {
    if (sellers.length === 0) {
      return 'No sellers yet. Send the first invite.';
    }
    const active = sellers.filter((r) => r.onboard);
    if (active.length === 0) {
      return `${sellers.length} invited. Nobody onboard yet.`;
    }
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-56 md:pb-24">
      ...
      {members.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-12 text-center">
          <p className="text-sm text-foreground">No sellers yet.</p>
