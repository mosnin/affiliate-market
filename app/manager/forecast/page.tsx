...
export const metadata: Metadata = { title: 'Revenue Forecast — Company' };
...
// set; otherwise fall back to the company's defaultManagerRate (same proxy
...
//   1. deal.commissionRate (set per-deal by the seller)
//   2. company.defaultManagerRate (company-wide default, ~ 2%)
...
export default async function ManagerForecastPage() {
  ...
  // Space to seller name lookup
  const spaceToSeller = new Map<string, string>();
  for (const m of allMembers) {
    const sid = m.Space?.id;
    if (!sid) continue;
    spaceToSeller.set(sid, m.User?.name ?? m.User?.email ?? 'Unknown');
  }
  ...
  if (spaceIds.length === 0) {
    return <EmptyPage companyName={company.name} monthName={monthName} />;
  }
  ...
  // Fetch closed deals (won + lost, all time) for per-seller conversion and
  ...
  // the per-seller rollup sees the full pipeline, not just the closed tail.
  ...
  let wonGci = 0;
  let wonCount = 0;
  // Realized GCI this month, per seller space — feeds the per-seller rollup.
  ...
  for (const d of (wonRaw ?? []) as Array<{
    ...
    const rate =
      d.commissionRate != null ? d.commissionRate / 100 : defaultManagerRate;
  ...
  for (const d of (activeRaw ?? []) as DealRow[]) {
    ...
    const sellerName = spaceToSeller.get(d.spaceId) ?? 'Unknown';
  ...
  // ── Per-seller performance context ────────────────────────────────────────
  // For each seller space we fold together: their active deals (from
  ...
  type SellerMetricRow = DealMetricRow & { spaceId: string };

  const metricRowsBySpace = new Map<string, SellerMetricRow[]>();
  const pushRow = (row: SellerMetricRow) => {
  ...
  const sellerRollups: SellerRollup[] = Array.from(sellerSpaceIds)
    ...
    // Show sellers with anything in flight or realized this month first; drop
  ...
  // Run across the company's active deals. Stages come from the same set we
        ...
        <>
          ...
          <section className="space-y-1">
            ...
            <p className={cn(CAPTION, 'tabular-nums')}>
              ...
              {defaultManagerRate != null && (
                <> &middot; default rate {Math.round(defaultManagerRate * 100)}%</>
              )}
            </p>
          </section>
          ...
          <section>
            ...
            {swingPool.length === 0 ? (
              ...
              <ul className="divide-y divide-border/60">
                {swingPool.map((deal) => {
                  const colaPrompt = `What can we do to close "${deal.title}" (${deal.sellerName}) this month?`;
                  const colaHref = `/manager?prompt=${encodeURIComponent(colaPrompt)}`;
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <div className="flex items-center flex-wrap gap-x-3 gap-y-0.5">
                            <span className={META}>{deal.sellerName}</span>
                        ...
                        <div className="flex-shrink-0 text-right space-y-1.5">
                          ...
                          <Link
                            href={colaHref}
                            className={cn(COLA_PILL, 'h-7 px-3 text-[11px]')}
                          >
                            Ask Cola
          ...
          <section>
            <div className="flex items-center gap-3 pb-3 border-b border-border/60">
              <h2 className={SECTION_LABEL}>By seller</h2>
              {sellerRollups.length > 0 && (
                <span className="text-[11px] text-muted-foreground tabular-nums">
                  {sellerRollups.length}
                </span>
              )}
            </div>
            ...
            {sellerRollups.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center mt-4">
                <p className="text-sm text-foreground">No seller activity yet.</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Each seller&rsquo;s forecast and pace will appear here as deals move.
                </p>
              </div>
            ) : (
              <ul className="divide-y divide-border/60">
          ...
          <p className={cn(CAPTION, 'text-muted-foreground/70')}>
            ...
            5% by health. Commission source: per-deal rate when set, otherwise company default
            ({Math.round(defaultManagerRate * 100)}%).
          </p>
        </>
      )}
    </div>
...
// ── Empty state when company has no members/spaces ─────────────────────────
...
function EmptyPage({
  ...
  return (
    <div className={cn('max-w-5xl mx-auto pb-12', SECTION_RHYTHM)}>
      <header className="space-y-1.5">
        <p className={BODY_MUTED}>{companyName}.</p>
        <h1 className={H1} style={TITLE_FONT}>
          {monthName} revenue
        </h1>
      ...
      <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
        <p className="text-sm text-foreground">Nothing in the pipeline yet.</p>
        <p className="text-xs text-muted-foreground mt-1">
          Invite your first seller and their deals will appear here.
        </p>
      </div>
    </div>
  );
