...
export function PipelineClient({ deals, stages, sellers, summary }: Props) {
  ...
  const filteredDeals = useMemo(() => {
    return deals.filter((d) => {
      ...
      if (sellerFilter !== 'all' && d.agentUserId !== sellerFilter) return false;
      return true;
    });
  }, [deals, statusFilter, sellerFilter]);
  ...
  return (
    <div className="space-y-10">
      ...
      {/* the manager home commission grid: gap-px on a border-bg makes  */}
      ...
      <section className="space-y-4">
        ...
        {agentGroups.length === 0 ? (
          <DashedEmpty>No sellers yet.</DashedEmpty>
        ) : (
          <ul className="divide-y divide-border/60">
            {agentGroups.map((agent) => {
              const isExpanded = expandedSellers.has(agent.userId);
              ...
              return (
                <li key={agent.userId}>
                  <button
                    onClick={() => toggleSeller(agent.userId)}
...
/**
 * Canonical dashed empty container — matches the manager home placeholder
 * vocabulary. Keep the copy first-person so Cola stays the narrator.
 */
