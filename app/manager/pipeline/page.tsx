...
export default async function ManagerPipelinePage() {
  ...
  // Get all seller members with their spaces
  const allMembers = await getCompanyMembers(company.id, { includeSpaceName: true });
  const members = allMembers.filter((m) => m.role === 'seller_member');

  const spaceIds = members.map((m) => m.Space?.id).filter(Boolean) as string[];

  // Build space -> seller lookup
  const spaceToSeller = new Map<string, { userId: string; name: string }>();
  const sellers: SellerInfo[] = [];
  for (const m of members) {
    const name = m.User?.name ?? m.User?.email ?? 'Unknown';
    if (m.Space?.id) {
      spaceToSeller.set(m.Space?.id, { userId: m.userId, name });
    }
    sellers.push({ userId: m.userId, name });
  }

  // Fetch all deals across seller spaces. The health classifier needs
  ...
  // Fetch all deal stages across seller spaces
  ...
  // deals" at a glance without the manager pivoting through every card.
  ...
  for (const d of (dealsRaw ?? []) as DealRow[]) {
    const seller = spaceToSeller.get(d.spaceId);
    ...
    const deal: PipelineDeal = {
      ...
      agentName: seller?.name ?? 'Unknown',
      agentUserId: seller?.userId ?? '',
    ...
    if (health.state === 'at-risk' || health.state === 'stuck') {
      ...
      const key = seller?.userId ?? 'unknown';
