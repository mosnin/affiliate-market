...
/**
 * Lead-score → tier. Mirrors contact-table.tsx and manager-leads-client.tsx.
...
export function ManagerPeopleTable() {
  ...
  const fetchContacts = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/manager/contacts?${params}`);
      if (!res.ok) {
        setError(true);
        return;
      }
      ...
      console.error('[manager-people-table] fetch failed:', err);
      setError(true);
    } finally {
      setLoading(false);
    }
  ...
  const subtitle = (() => {
    ...
    if (newThisWeekCount > 0) {
      return `${contacts.length} ${noun} across the company. ${newThisWeekCount} new this week.`;
    }
    return `${contacts.length} ${noun} across the company.`;
  })();
    ...
    <div className="space-y-6">
      ...
      <header className="space-y-1.5">
        ...
        <h1 className={H1} style={TITLE_FONT}>
          Everyone your company is working with
        </h1>
        ...
        if (!search && !hasAnyFilter && contacts.length === 0) {
          return (
            <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-12 text-center">
              <p className="text-base text-foreground">No contacts across the company yet.</p>
              <p className={cn(BODY_MUTED, 'mt-1.5')}>
                Contacts will appear here as your sellers add them.
              </p>
            </div>
          );
        }
...
// manager-only addition: a Seller byline under the contact's name/email line.
// Row click drills into /manager?prompt=... (Cola prefilled about this lead).
...
function ManagerContactRow({
  ...
  const drillPrompt = encodeURIComponent(
    `Tell me about ${contact.name} (${contact.sellerName}'s lead). Where are they in the funnel and what's the next move?`
  );
  ...
  return (
    ...
    >
      <Link href={`/manager?prompt=${drillPrompt}`} className={rowClassName}>
        {/* Avatar */}
        ...
        {/* Name + stage pill + email/phone + seller byline */}
        <div className="min-w-0 flex-1">
          ...
          {/* Seller byline — the manager-only addition */}
