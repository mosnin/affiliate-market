...
export function ReviewsClient({ initialReviews, initialOpenCount, role, companyName }: Props) {
  ...
  // somewhere in this company. Best-effort: derived from the cached
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/manager/reviews?status=${tab}`, {
          cache: 'no-store',
        });
