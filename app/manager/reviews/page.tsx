...
//
// Use getManagerContext (not requireManager) so non-managers get a clean
// redirect instead of an uncaught throw — matches app/manager/members/page.tsx.
export default async function ManagerReviewsPage() {
  const ctx = await getManagerContext();
  if (!ctx) redirect('/');

  // 1. Pull open review requests for this company, newest-first. Other tabs
  //    fetch on demand via /api/manager/reviews?status=X.
  ...
  // Headline status — calm, factual. Mirrors the manager overview pattern
