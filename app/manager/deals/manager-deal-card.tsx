 ...
 * Manager kanban card.
 *
 * Mirrors the seller DealCard visual vocabulary exactly — same layout,
 * same serif value, same health dot, same days-in-stage line — with two
 * additions: a seller byline below the address, and no drag handle (the
 * manager is read-only, no restaging). The card is intentionally a replica,
...
export interface ManagerDealItem {
  id: string;
  title: string;
  address: string | null;
  value: number | null;
...
export function ManagerDealCard({
  deal,
  entranceIndex = null,
  onClick,
}: ManagerDealCardProps) {
  const staggerDelay =
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Focal value — serif Times, matches seller card */}
        {deal.value != null && (
          <p
        ...
        {/* Days in stage — manager-specific signal */}
        ...
        {/* Seller byline — the manager oversight addition */}
