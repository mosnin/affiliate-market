...
import { ManagerKanbanBoard } from './manager-kanban-board';
import type { ManagerDealItem } from './manager-deal-card';
import type { ManagerKanbanColumn } from './manager-kanban-board';
import type { Metadata } from 'next';

export const metadata: Metadata = { title: 'Deals — Company' };
 ...
 *
 * Because each seller can have custom stage names, we build columns from
 ...
 *     stage?" when sellers name them the same.
 ...
 *
 * The result: the manager sees a kanban whose column order mirrors the
 * seller pipeline direction even across mixed stage names.
 */
function buildColumns(
  stages: StageRow[],
  dealsWithSeller: Array<ManagerDealItem & { stageId: string }>,
): ManagerKanbanColumn[] {
  ...
  const columnMap = new Map<string, ManagerKanbanColumn>();
  ...
  const unmapped: Array<ManagerDealItem & { stageId: string }> = [];
  for (const deal of dealsWithSeller) {
  ...
  // Drop empty columns — a manager pipeline with all-empty columns is noise.
...
export default async function ManagerDealsPage() {
  ...
  // Resolve ALL company members — same pattern as the previous page.
  ...
  // Build spaceId to seller name lookup
  ...
  // Won/lost/on-hold deals are closed chapter; the manager oversight
  ...
  // ── Enrich deals with seller names ──────────────────────────────────────

  const allDeals: Array<ManagerDealItem & { stageId: string }> = [];
  ...
  for (const d of (dealsRaw ?? []) as DealRow[]) {
    ...
    allDeals.push({
      ...
      nextActionDueAt,
      sellerName,
      // Pass stageId for column slotting; not in ManagerDealItem interface
      stageId: d.stageId,
    });
  }
  ...
  const statusSentence = (() => {
    if (activeCount === 0) return 'No active deals across your company yet.';
    const parts: string[] = [];
    parts.push(
  ...
  return (
    <div className={cn('max-w-[1500px] mx-auto pb-12', SECTION_RHYTHM)}>
      ...
      <header className="space-y-1.5">
        <p className={BODY_MUTED}>Company.</p>
      ...
      <ManagerKanbanBoard columns={columns} />
    </div>
  );
}
