...
import { ManagerMorningStory } from '@/components/manager/manager-morning-story';
import { DraftImpactCard } from '@/components/manager/draft-impact-card';
import {
  aggregateDraftStats,
...
// ── Revenue forecast helpers (mirrors app/manager/forecast/page.tsx) ──────────
//
...
export default async function ManagerBriefPage() {
  ...
  // seller_member sees their own work surface, not the swarm.
  if (ctx.membership.role === 'seller_member') {
    return <MemberDashboard ctx={ctx} />;
  }

  const { company } = ctx;
  const members = await getCompanyMembers(ctx.company.id, {
    includeOnboard: true,
  ...
  // Make sure the manager owner's space is included — company leads land
  ...
  // updatedAt falls in the window. Mirrors `agent/tools/manager/revenue.py`
  ...
  // Top seller MTD — owner name keyed off the spaceId → space.ownerId map
  // from the `members` rows we already fetched. We only label the cell when
  // someone actually earned non-zero GCI this month.
  let topSellerMtd: { name: string; amount: number } | null = null;
  ...
  if (topSpaceId) {
    ...
    const name = owner?.User?.name?.split(' ')[0] ?? owner?.User?.name ?? 'Seller';
    topSellerMtd = { name, amount: topAmount };
  }

  // ── Revenue forecast (brief read) ─────────────────────────────────────────
  // Reuses the forecast model from app/manager/forecast/page.tsx.
  // Won GCI: same MTD window + defaultManagerRate fallback already computed above.
  // Active GCI: weighted by closingProbability × dealHealth — one extra query.

  const b = company as unknown as { defaultManagerRate?: number | null };
  const defaultManagerRate =
    typeof b.defaultManagerRate === 'number' ? b.defaultManagerRate : 0.02;
  ...
  for (const d of wonYtdRows) {
    const updatedMs = new Date(d.updatedAt).getTime();
    if (updatedMs >= mtdStartMs) {
      const rate = d.commissionRate != null ? d.commissionRate / 100 : defaultManagerRate;
  ...
  // Two numbers: leads currently breaching (needs action) + leads Cola
  // escalated to the manager today (proof of work).
  let slaEnabled = false;
  let needsResponse = 0;
  let escalatedToday = 0;
  if (company.slaEnabled && spaceIds.length > 0) {
    slaEnabled = true;
  ...
  const hasSettings = !!(company.name && (company.logoUrl || company.websiteUrl));

  // Sort members by current activity load (active drafts + new leads + active deals)
  // so the manager scans the busiest sellers first. Owner row stays pinned to top.
  const memberRows = members
    ...
    .sort((a, b) => {
      ...
      if (a.member.role === 'manager_owner' && b.member.role !== 'manager_owner') return -1;
      if (b.member.role === 'manager_owner' && a.member.role !== 'manager_owner') return 1;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 pb-12">
      {/* Header — canonical three-line pattern. Muted greeting (with the
          team name as the calm anchor), serif Times h1 from ManagerMorningStory
          carrying Cola's chief-of-staff sentence, and the small hairline
      ...
      {/* Cola — the focal entry. The manager's chief of staff is the home of
          this page, not its fourth section. A calm, paper-flat doorway: serif
          name, one line of what it does, a quiet pill into /manager/cola.
      ...
      {/* Snapshot — three numbers the manager actually cares about.
      ...
      {/* Commission — money is the loudest signal. Hairline-divider grid
          ...
          drills into /manager/commissions for the per-deal breakdown. */}
      <section>
        <Link
      ...
      {/* Revenue — projected GCI for the month at a glance.
          ...
          deal to watch. Computation mirrors app/manager/forecast/page.tsx. */}
      <section>
        <Link
              ...
              <>
                ...
                {escalatedToday > 0 && (
                  <span className="text-muted-foreground">
                    {' '}Cola escalated {escalatedToday} {escalatedToday === 1 ? 'lead' : 'leads'} to you today.
                  </span>
                )}
              </>
            ) : escalatedToday > 0 ? (
              <span className="text-muted-foreground">
                Every lead has been answered. Cola escalated {escalatedToday} {escalatedToday === 1 ? 'lead' : 'leads'} to you today.
              </span>
            ) : (
        ...
        <section>
          ...
          <ul className="divide-y divide-border/60">
            {pendingInvitations.map((inv) => {
              const role =
                inv.roleToAssign === 'manager_owner'
                  ? 'Owner'
        ...
        {memberRows.length === 0 ? (
          ...
          <ul className="divide-y divide-border/60">
            {memberRows.map(({ member, leads, deals, drafts, apps }) => {
              const name = member.User?.name ?? 'Unnamed seller';
              const initial = name.charAt(0).toUpperCase();
              // A teammate row opens the manager-scoped seller detail page.
              // /s/<slug>/* requires owning that space, so a manager can't open a
              // teammate's own dashboard — it 404s. /manager/sellers/<userId> is
              // the manager's view of that seller (the same link the Sellers
              // list uses). The manager's own chief of staff lives at /manager/cola.
              const href = member.userId ? `/manager/sellers/${member.userId}` : '#';
              const role =
                member.role === 'manager_owner'
                  ? 'Owner'
                  : member.role === 'manager_admin'
                    ? 'Admin'
                    : 'Seller';
      ...
      {/* Draft impact — how Cola's drafts have actually been landing across
          the team over the last 30 days. Sits below the team list so the
          seller row remains the focal element of the page. */}
