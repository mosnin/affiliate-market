...
export const metadata: Metadata = { title: 'Billing — Teams — Cola' };
...
export default async function ManagerBillingPage() {
  ...
  // The company's OWN Stripe identity — written by the company-scoped
  ...
  if (!spaceRow && !usingCompanyEntity) {
    return (
  ...
  // Only the manager_owner can manage billing. Others see a read-only message.
  const isManagerOwner = ctx.membership.role === 'manager_owner';
  if (!isManagerOwner) {
    return (
      <div className="space-y-6 max-w-3xl mx-auto pb-12">
        <BillingHeader status="The company owner manages billing." />
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          <p className="text-sm text-foreground">Billing lives with the owner.</p>
          <p className="text-xs text-muted-foreground mt-1">
            Ask the company owner to make changes to the subscription.
          </p>
        </div>
      </div>
    );
  ...
  // Billing entity: the Company's subscription when it has one, else the
  // legacy owner-space subscription. The previous version of this page ONLY
  // read the owner's personal Space, so a company-scoped subscription showed
  // the wrong status/invoices/card here.
  const billingSubscriptionId = usingCompanyEntity
    ? (companyStripe!.stripeSubscriptionId as string)
    : ((spaceRow?.stripeSubscriptionId as string | null) ?? null);
  const billingCustomerId = usingCompanyEntity
    ? ((companyStripe!.stripeCustomerId as string | null) ?? null)
    : ((spaceRow?.stripeCustomerId as string | null) ?? null);

  const slug = (spaceRow?.slug as string | undefined) ?? '';
  let subscriptionStatus: 'active' | 'trialing' | 'past_due' | 'canceled' | 'inactive' = usingCompanyEntity
    ? mapDbStatus(companyStripe?.stripeSubscriptionStatus)
  ...
  if (billingSubscriptionId) {
    try {
      ...
      console.error('[manager/billing] Stripe fetch failed:', err);
      stripeError = true;
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-12">
      ...
      <BillingPage
        slug={slug}
        plan={(((ctx.company as { plan?: string }).plan) ?? 'free') as PlanId}
        subscriptionStatus={subscriptionStatus}
        currentPeriodEnd={currentPeriodEnd}
        cardLast4={cardLast4}
        cardBrand={cardBrand}
        invoices={invoices}
        // Manage/cancel must act on the company's Stripe identity when the
        // company holds the subscription — the default routes act on a Space
        // the caller owns, which is the wrong entity for a company sub.
        endpoints={
          usingCompanyEntity
            ? { portal: '/api/manager/billing/portal', cancel: '/api/manager/billing/cancel' }
            : undefined
        }
      />
      {/* Credits are keyed to the runtime space; a company-entity manager
