...
export async function MemberDashboard({ ctx }: MemberDashboardProps) {
  ...
  const userName = userRow?.name ?? userRow?.email ?? 'Seller';
      ...
      <div className="space-y-6 w-full">
        <header className="space-y-1.5">
          ...
          <p className={cn(BODY_MUTED)}>
            {`${company.name} · finish your workspace to start tracking leads.`}
          </p>
        </header>
  ...
  // Resolve the company's admin/owner spaces FIRST so the announcement
  ...
  // rows over the wire.
  const managerMembers = await getCompanyMembers(company.id);
  const managerSpaceIds = Array.from(
    new Set(
      managerMembers
        .filter((m) => m.role === 'manager_owner' || m.role === 'manager_admin')
        .map((m) => m.Space?.id)
  ...
  const [
    ...
    // Count leads assigned by manager
    spaceId
      ? supabase
          .from('Contact')
  ...
  // Announcements are already server-scoped to managerSpaceIds and limit(3).
  ...
  const statusSentence = (() => {
    ...
    if (parts.length === 0) {
      return `${company.name} · quiet day — nothing in flight.`;
    }
    return `${company.name} · ${parts.join(' · ')}.`;
  })();
