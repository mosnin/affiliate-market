...
Cola is well past initial build. The CRM substrate, public intake, lead scoring, demos, the company tier, and the autonomous agent are all shipped and in use.
...
| Integrations | Composio toolkits as agent tools (Gmail, HubSpot, Slack, Calendar); Cola exposed as an MCP server |
| Chat / cockpit UX | The seller's chat surface — live status, tool-call rendering, the instrument/cockpit pass |
| Billing & usage | Per-seat company billing is in place; usage metering and plan enforcement for the agent are being built |
