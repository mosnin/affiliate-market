...
Agentic operating system for sellers and companies.

---

## What is Cola?

Cola is an agentic operating system for sellers and companies. It combines always-on AI agent workflows, lead intake, qualification, follow-up orchestration, and company coordination into a single operational platform.
...
- **Company management** — Multi-user team dashboards, invite system, performance tracking across sellers
- **AI agent** — Agent runtime with tool-use over the seller operating system (read-only tools auto-run; mutating tools — email, SMS, deal/stage changes, demos — require per-call user approval). Delegates research questions to read-only sub-agents so profile lookups don't bloat the orchestrator's context. See `lib/ai-tools/tools/index.ts` for the tool registry and `lib/ai-tools/skills/*` for the sub-agents.
- **Always-on background activation** — Incoming workspace events (new lead, deal stage change, demo completed, application submitted) are queued in Redis and immediately attempt a Modal webhook fire (`POST /api/agent/trigger`) so per-seller agents can react in near real-time with queue-based fallback if Modal is unavailable. Immediate fire policy is configurable with `AGENT_IMMEDIATE_EVENTS` (`all` by default, or comma-separated event names; invalid values fail safe to `all`).
...
- **Company tier** — Multi-agent organisation with per-seat billing: company membership + role tiers (`manager_owner`, `manager_admin`, `seller_member`) in `lib/permissions.ts`; lead routing across agents (`lib/company-routing.ts`); commission ledger (`lib/commissions.ts`); Stripe-backed seat subscriptions (`lib/company-seats.ts`, `app/api/billing/*`)
- **Notifications** — Email (Resend) and SMS (Telnyx) notifications for leads, demos, deals, and follow-ups
- **Analytics** — Weekly trends, conversion funnels, and team performance metrics

### Who it's for

- **Solo sellers** handling leasing and rental leads
- **Small teams** and companies managing multiple sellers
...
app/                    # Next.js App Router pages, layouts, API routes
  ...
  manager/               # Company management pages
...
components/             # UI and feature components
  ...
  manager/               # Company-specific components
...
1. **Seller signs up** via Clerk and completes onboarding (or skips to set up later)
...
5. **Seller manages** leads, contacts, deals, and demos from the workspace dashboard
6. **Notifications sent** via email and/or SMS based on workspace preferences
7. **Managers** can invite sellers, track team performance, and manage the company
