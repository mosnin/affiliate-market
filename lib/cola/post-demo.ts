/**
 * Post-demo orchestration helpers.
 *
 ...
 * ask the model to propose 2-5 actions the seller would have taken
 * manually. The model does NOT execute the tools — it returns intent
 * only. The seller approves the batch in one tap; the route then runs
 ...
 *    seller who wants to review before anything happens.
 ...
 * truncate to 5. Either failure mode is the model's fault; the seller
...
/** Subset of tools the orchestrator is allowed to propose post-demo. The
 *  full 42-tool registry is too noisy for a 30-second debrief — most of
 *  these are administrative or aggregate queries the seller would never
 *  trigger from a demo recap. The seven below cover the actual moments:
 *  log it, mark it, schedule the next thing, draft the message. */
export const POST_DEMO_TOOL_ALLOWLIST = [
  'log_call',
  'log_meeting',
  'note_on_person',
  'note_on_deal',
...
export type PostDemoToolName = (typeof POST_DEMO_TOOL_ALLOWLIST)[number];

/**
 * Composio slug allowlist for integration tools the post-demo proposer is
 ...
 * `/api/cola/post-demo/execute` with `tool: "GMAIL_TRASH_EMAIL"` (or any
 ...
 */
export const POST_DEMO_INTEGRATION_SLUG_ALLOWLIST = [
  'GMAIL_SEND_EMAIL',
  'GOOGLECALENDAR_CREATE_EVENT',
] as const;

export type PostDemoIntegrationSlug =
  (typeof POST_DEMO_INTEGRATION_SLUG_ALLOWLIST)[number];

/**
 * A Composio tool the seller's connected accounts expose. We pull this
 ...
 * for the done-verb resolver, NOT shown to the seller in the proposal
...
export interface IntegrationToolSpec {
  ...
  slug: string;
  /** Seller-readable description. May be Composio's text, may be pre-processed. */
  description: string;
...
export interface ProposedAction {
  ...
  tool: PostDemoToolName | string;
  args: Record<string, unknown>;
  /** Human-readable line — the row label in the approval stack. */
  summary: string;
  /** Cola-voice line that uses the resolved Contact/Deal name instead of
  ...
  humanSummary?: string;
  /** Whether this verb mutates state (i.e. needs the seller's yes). */
  mutates: boolean;
  /**
 ...
 *
 *  When the seller has connected apps (Gmail, Google Calendar, ...),
...
export function buildSystemPrompt(
  ...
  const lines: string[] = [
    'Turn a seller\'s post-demo debrief into 2-5 proposed actions. Return intent only — never execute.',
    '',
    'Allowed tools:',
    native,
  ];
  ...
  if (integrationTools.length > 0) {
    ...
    lines.push(
      '',
      'Connected-app tools (the seller has authorized these):',
      integ,
      '',
      'When the seller\'s intent is to actually send, schedule, or post — not draft — prefer the connected-app tool.',
      'Examples: "send Sam a follow-up" → GMAIL_SEND_EMAIL (if Gmail connected). "put the demo on my calendar" → GOOGLECALENDAR_CREATE_EVENT.',
      'If the seller only wants to see what they\'d say (a draft), use the draft_* tools instead.',
    );
  }
 ...
 *  the in-process registry; integration tools come from the seller's
...
export function shapeProposals(
  ...
  const nativeAllow = new Set(opts.allowlist ?? POST_DEMO_TOOL_ALLOWLIST);
  ...
  for (const item of parsed.data.proposals) {
    ...
    if (nativeAllow.has(item.tool)) {
      ...
      out.push({
        tool: item.tool as PostDemoToolName,
        args: item.args,
        summary,
...
/** Seller-voice summary for an integration proposal. The toolkit slug
...
function integrationProposalSummary(
  ...
  const verb = sellerVerbForToolkit(toolkit);
...
export async function proposeActions(
  ...
  const allowed = input.tools.filter((t) =>
    (POST_DEMO_TOOL_ALLOWLIST as readonly string[]).includes(t.name),
  );
...
/**
 * Resolve the Composio tools the seller's post-demo orchestrator should
 ...
 * native catalog and the seller never sees the seam.
 *
 ...
 */
export async function loadPostDemoIntegrationTools(args: {
  spaceId: string;
  userId: string;
  ...
  try {
    ...
    logger.warn(
      '[post-demo] integration tools load failed — proceeding without them',
 ...
 *  `GMAIL_SEND_EMAIL` → `gmail`. Cross-checks against the seller's
 ...
 * write a Cola-voice `humanSummary` onto each one. Names replace ID
 ...
 *
 * The post-demo ceremony is the bid-for-10 surface; chat-side approval
...
export function formatHumanSummary(
  ...
  // Integration proposals come from connected apps. The seller doesn't
  ...
  if (proposal.integrationToolkit) {
    const verb = sellerVerbForToolkit(proposal.integrationToolkit);
...
/**
 * Seller-voice verb for the proposal stack. The seller doesn't know
 ...
 */
export function sellerVerbForToolkit(toolkit: string): string {
  switch (toolkit) {
 ...
 * the seller reads this on the way back to their car. Falls back to a
