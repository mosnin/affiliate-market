/**
 * Cola-voiced helpers for the chat path.
 *
 ...
 *
 *   - colaErrorMessage: maps an error code to a first-person Cola line so
 *     a tool failure or backend hiccup reads like Cola talking, not like a
 *     stack trace leaking through.
 *
 * `colaErrorMessage` / `classifyError` are sync, side-effect free, and safe
 ...
 *
 * Strategy:
 *   1. Strip HTML, control chars, mention prefix from cola-workspace.
...
/**
 * The single source of truth for what Cola says when something goes wrong.
 * First-person, in-character, no system-warning vocabulary.
 */
export function colaErrorMessage(code: ColaErrorCode): string {
  switch (code) {
    case 'cold_start':
...
/**
 * Best-effort classification of a raw error message into a ColaErrorCode.
 * Used at points where we only have a string (e.g. a fetch reject reason)
 * and need to pick the right Cola line.
 */
export function classifyError(raw: string | undefined | null): ColaErrorCode {
