 ...
 * via chat completions, with the seller's workspace model, the full tool set,
 ...
 *
 * `'modal'` — opt-in (COLA_CHAT_RUNTIME=modal). Proxies the WHOLE chat turn
...
export function chatRuntime(): ChatRuntime {
  const v = process.env.COLA_CHAT_RUNTIME;
