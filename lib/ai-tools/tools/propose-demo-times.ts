/**
 * `propose_demo_times` — suggest 4-6 open 60-minute slots on the seller's
 * calendar for the next 7 days during business hours. The chat renders
 * these as a tappable picker; the seller picks one → the workspace sends
 * a "Schedule the demo at <slot>" prompt → Cola fires `schedule_demo`
...
const parameters = z
  .object({
    contactId: z
      ...
      .min(1)
      .optional()
      .describe('Saved Contact.id this demo is for. Surface their name on the picker.'),
    productAddress: z
      .string()
      .min(1)
      .optional()
      .describe('Product address the demo is for. Surface on the picker.'),
    durationMinutes: z
      .number()
      .int()
      .min(15)
      .max(240)
      .optional()
      .describe('Demo length. Defaults to 60.'),
    daysAhead: z
      .number()
      .int()
      .min(1)
  ...
  .describe(
    'Propose 4-6 open slots in business hours over the next week. Renders as a tappable picker — the seller picks a slot to fire schedule_demo.',
  );
...
interface ProposeDemoTimesResult {
  slots: ProposedSlot[];
  contactId?: string;
...
/** Business hours window in the seller's local time. Lean toward
 *  showings: skip pre-9am, skip after 6pm, skip weekends. */
const BUSINESS_START_HOUR = 9;
const BUSINESS_END_HOUR = 18;

export const proposeDemoTimesTool = defineTool<typeof parameters, ProposeDemoTimesResult>({
  name: 'propose_demo_times',
  riskLevel: 'safe',
  description:
    'Suggest 4-6 open demo times for the seller over the next week. Returns slots the seller can tap to schedule.',
  parameters,
