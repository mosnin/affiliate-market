...
export interface ErrorEvent extends BaseEvent {
  type: 'error';
  /**
   * Human-facing message. The server replaces this with a Cola-voiced line
   * (see lib/ai-tools/cola-voice.ts) before shipping; the client renders
   * it inline as an assistant message rather than as a system warning.
   */
  message: string;
  /**
   * Machine-readable code so the client can pick its own Cola line if the
